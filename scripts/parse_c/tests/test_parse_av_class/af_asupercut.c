# 1 "/Users/davidchen/repo/ffmpeg/libavfilter/af_asupercut.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 418 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/Users/davidchen/repo/ffmpeg/libavfilter/af_asupercut.c" 2
# 22 "/Users/davidchen/repo/ffmpeg/libavfilter/af_asupercut.c"
...
# 23 "/Users/davidchen/repo/ffmpeg/libavfilter/af_asupercut.c" 2
...
# 24 "/Users/davidchen/repo/ffmpeg/libavfilter/af_asupercut.c" 2
# 1 "./libavutil/opt.h" 1
# 31 "./libavutil/opt.h"
...
int av_opt_serialize(void *obj, int opt_flags, int flags, char **buffer,
                     const char key_val_sep, const char pairs_sep);
# 25 "/Users/davidchen/repo/ffmpeg/libavfilter/af_asupercut.c" 2
...
# 26 "/Users/davidchen/repo/ffmpeg/libavfilter/af_asupercut.c" 2
...
# 27 "/Users/davidchen/repo/ffmpeg/libavfilter/af_asupercut.c" 2
...
static int get_coeffs(AVFilterContext *ctx)
{
    ...
    double K = tan(3.14159265358979323846264338327950288 * w0);
    double q[10];
    ...
    if (!strcmp(ctx->filter->name, "asubcut")) {
        ...
        if (s->order & 1) {
            ...
            double omega = 2. * tan(3.14159265358979323846264338327950288 * w0);
        ...
        double theta_0 = 2. * 3.14159265358979323846264338327950288 * (s->cutoff / inlink->sample_rate);
        double d_E;
        ...
        for (int b = 0; b < s->filter_count; b += 2) {
            double D = 2. * sin(((b + 1) * 3.14159265358979323846264338327950288) / (2. * s->filter_count));
...
# 244 "/Users/davidchen/repo/ffmpeg/libavfilter/af_asupercut.c"
static int filter_channels_fltp(AVFilterContext *ctx, void *arg, int jobnr, int nb_jobs) { ASuperCutContext *s = ctx->priv; ThreadData *td = arg; AVFrame *out = td->out; AVFrame *in = td->in; const int start = (in->ch_layout.nb_channels * jobnr) / nb_jobs; const int end = (in->ch_layout.nb_channels * (jobnr+1)) / nb_jobs; const double level = s->level; for (int ch = start; ch < end; ch++) { const float *src = (const float *)in->extended_data[ch]; float *dst = (float *)out->extended_data[ch]; for (int b = 0; b < s->filter_count; b++) { BiquadCoeffs *coeffs = &s->coeffs[b]; const float a1 = coeffs->a1; const float a2 = coeffs->a2; const float b0 = coeffs->b0; const float b1 = coeffs->b1; const float b2 = coeffs->b2; float *w = ((float *)s->w->extended_data[ch]) + b * 2; for (int n = 0; n < in->nb_samples; n++) { float sin = b ? dst[n] : src[n] * level; float sout = sin * b0 + w[0]; w[0] = b1 * sin + w[1] + a1 * sout; w[1] = b2 * sin + a2 * sout; dst[n] = sout; } } } return 0; }
static int filter_channels_dblp(AVFilterContext *ctx, void *arg, int jobnr, int nb_jobs) { ASuperCutContext *s = ctx->priv; ThreadData *td = arg; AVFrame *out = td->out; AVFrame *in = td->in; const int start = (in->ch_layout.nb_channels * jobnr) / nb_jobs; const int end = (in->ch_layout.nb_channels * (jobnr+1)) / nb_jobs; const double level = s->level; for (int ch = start; ch < end; ch++) { const double *src = (const double *)in->extended_data[ch]; double *dst = (double *)out->extended_data[ch]; for (int b = 0; b < s->filter_count; b++) { BiquadCoeffs *coeffs = &s->coeffs[b]; const double a1 = coeffs->a1; const double a2 = coeffs->a2; const double b0 = coeffs->b0; const double b1 = coeffs->b1; const double b2 = coeffs->b2; double *w = ((double *)s->w->extended_data[ch]) + b * 2; for (int n = 0; n < in->nb_samples; n++) { double sin = b ? dst[n] : src[n] * level; double sout = sin * b0 + w[0]; w[0] = b1 * sin + w[1] + a1 * sout; w[1] = b2 * sin + a2 * sout; dst[n] = sout; } } } return 0; }
...
static int filter_frame(AVFilterLink *inlink, AVFrame *in)
{
    ...
    td.in = in; td.out = out;
    ff_filter_execute(ctx, s->filter_channels, &td, ((void*)0),
...
static const AVOption asupercut_options[] = {
    { "cutoff", "set cutoff frequency", __builtin_offsetof(ASuperCutContext, cutoff), AV_OPT_TYPE_DOUBLE, {.dbl=20000}, 20000, 192000, 8|(1<<16)|(1<<15) },
    { "order", "set filter order", __builtin_offsetof(ASuperCutContext, order), AV_OPT_TYPE_INT, {.i64=10}, 3, 20, 8|(1<<16)|(1<<15) },
    { "level", "set input level", __builtin_offsetof(ASuperCutContext, level), AV_OPT_TYPE_DOUBLE, {.dbl=1.}, 0., 1., 8|(1<<16)|(1<<15) },
    { ((void*)0) }
};

static const AVClass asupercut_class = { .class_name = "asupercut", .item_name = av_default_item_name, .option = asupercut_options, .version = ((58)<<16 | (36)<<8 | (100)), .category = AV_CLASS_CATEGORY_FILTER, };
...
static const AVOption asubcut_options[] = {
    { "cutoff", "set cutoff frequency", __builtin_offsetof(ASuperCutContext, cutoff), AV_OPT_TYPE_DOUBLE, {.dbl=20}, 2, 200, 8|(1<<16)|(1<<15) },
    { "order", "set filter order", __builtin_offsetof(ASuperCutContext, order), AV_OPT_TYPE_INT, {.i64=10}, 3, 20, 8|(1<<16)|(1<<15) },
    { "level", "set input level", __builtin_offsetof(ASuperCutContext, level), AV_OPT_TYPE_DOUBLE, {.dbl=1.}, 0., 1., 8|(1<<16)|(1<<15) },
    { ((void*)0) }
};

static const AVClass asubcut_class = { .class_name = "asubcut", .item_name = av_default_item_name, .option = asubcut_options, .version = ((58)<<16 | (36)<<8 | (100)), .category = AV_CLASS_CATEGORY_FILTER, };
...
static const AVOption asuperpass_asuperstop_options[] = {
    { "centerf","set center frequency", __builtin_offsetof(ASuperCutContext, cutoff), AV_OPT_TYPE_DOUBLE, {.dbl=1000}, 2, 999999, 8|(1<<16)|(1<<15) },
    { "order", "set filter order", __builtin_offsetof(ASuperCutContext, order), AV_OPT_TYPE_INT, {.i64=4}, 4, 20, 8|(1<<16)|(1<<15) },
    { "qfactor","set Q-factor", __builtin_offsetof(ASuperCutContext, qfactor),AV_OPT_TYPE_DOUBLE, {.dbl=1.},0.01, 100., 8|(1<<16)|(1<<15) },
    { "level", "set input level", __builtin_offsetof(ASuperCutContext, level), AV_OPT_TYPE_DOUBLE, {.dbl=1.}, 0., 2., 8|(1<<16)|(1<<15) },
    { ((void*)0) }
};

static const AVClass asuperpass_asuperstop_class = { .class_name = "asuperpass/asuperstop", .item_name = av_default_item_name, .option = asuperpass_asuperstop_options, .version = ((58)<<16 | (36)<<8 | (100)), .category = AV_CLASS_CATEGORY_FILTER, };
