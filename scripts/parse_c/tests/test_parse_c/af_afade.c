# 1 "/Users/davidchen/repo/ffmpeg/libavfilter/af_afade.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 418 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/Users/davidchen/repo/ffmpeg/libavfilter/af_afade.c" 2
# 26 "/Users/davidchen/repo/ffmpeg/libavfilter/af_afade.c"
# 1 "./config_components.h" 1
# 27 "/Users/davidchen/repo/ffmpeg/libavfilter/af_afade.c" 2
...
int av_opt_serialize(void *obj, int opt_flags, int flags, char **buffer,
                     const char key_val_sep, const char pairs_sep);
# 29 "/Users/davidchen/repo/ffmpeg/libavfilter/af_afade.c" 2
...
# 30 "/Users/davidchen/repo/ffmpeg/libavfilter/af_afade.c" 2
...
# 32 "/Users/davidchen/repo/ffmpeg/libavfilter/af_afade.c" 2
...
static double fade_gain(int curve, int64_t index, int64_t range, double silence, double unity)
{
    ...
    gain = av_clipd_c(1.0 * index / range, 0, 1.0);

    switch (curve) {
    case QSIN:
        gain = sin(gain * 3.14159265358979323846264338327950288 / 2.0);
        break;
    case IQSIN:
        ...
        break;
    case ESIN:
        gain = 1.0 - cos(3.14159265358979323846264338327950288 / 4.0 * (((2.0*gain - 1)*(2.0*gain - 1)*(2.0*gain - 1)) + 1));
        break;
    case HSIN:
        gain = (1.0 - cos(gain * 3.14159265358979323846264338327950288)) / 2.0;
        break;
    case IHSIN:
    ...
    case EXP:
        ...
        break;
    case LOG:
        gain = av_clipd_c(1 + 0.2 * log10(gain), 0, 1.0);
        break;
    case PAR:
        gain = 1 - sqrt(1 - gain);
        break;
    ...
    case LOSI: {
        ...
        break;
    case SINC:
        gain = gain >= 1.0 ? 1.0 : sin(3.14159265358979323846264338327950288 * (1.0 - gain)) / (3.14159265358979323846264338327950288 * (1.0 - gain));
        break;
    case ISINC:
        gain = gain <= 0.0 ? 0.0 : 1.0 - sin(3.14159265358979323846264338327950288 * gain) / (3.14159265358979323846264338327950288 * gain);
        break;
    case QUAT:
    ...
    case QUATR:
        ...
        break;
    case QSIN2:
        gain = sin(gain * 3.14159265358979323846264338327950288 / 2.0) * sin(gain * 3.14159265358979323846264338327950288 / 2.0);
        break;
    case HSIN2:
        gain = pow((1.0 - cos(gain * 3.14159265358979323846264338327950288)) / 2.0, 2.0);
        break;
    case NONE:
        gain = 1.0;
        break;
...
# 201 "/Users/davidchen/repo/ffmpeg/libavfilter/af_afade.c"
static void fade_samples_dblp(uint8_t **dst, uint8_t * const *src, int nb_samples, int channels, int dir, int64_t start, int64_t range,int curve, double silence, double unity) { int i, c; for (i = 0; i < nb_samples; i++) { double gain = fade_gain(curve, start + i * dir,range,silence,unity); for (c = 0; c < channels; c++) { double *d = (double *)dst[c]; const double *s = (double *)src[c]; d[i] = s[i] * gain; } } }
static void fade_samples_fltp(uint8_t **dst, uint8_t * const *src, int nb_samples, int channels, int dir, int64_t start, int64_t range,int curve, double silence, double unity) { int i, c; for (i = 0; i < nb_samples; i++) { double gain = fade_gain(curve, start + i * dir,range,silence,unity); for (c = 0; c < channels; c++) { float *d = (float *)dst[c]; const float *s = (float *)src[c]; d[i] = s[i] * gain; } } }
static void fade_samples_s16p(uint8_t **dst, uint8_t * const *src, int nb_samples, int channels, int dir, int64_t start, int64_t range,int curve, double silence, double unity) { int i, c; for (i = 0; i < nb_samples; i++) { double gain = fade_gain(curve, start + i * dir,range,silence,unity); for (c = 0; c < channels; c++) { int16_t *d = (int16_t *)dst[c]; const int16_t *s = (int16_t *)src[c]; d[i] = s[i] * gain; } } }
static void fade_samples_s32p(uint8_t **dst, uint8_t * const *src, int nb_samples, int channels, int dir, int64_t start, int64_t range,int curve, double silence, double unity) { int i, c; for (i = 0; i < nb_samples; i++) { double gain = fade_gain(curve, start + i * dir,range,silence,unity); for (c = 0; c < channels; c++) { int32_t *d = (int32_t *)dst[c]; const int32_t *s = (int32_t *)src[c]; d[i] = s[i] * gain; } } }

static void fade_samples_dbl (uint8_t **dst, uint8_t * const *src, int nb_samples, int channels, int dir, int64_t start, int64_t range, int curve, double silence, double unity) { double *d = (double *)dst[0]; const double *s = (double *)src[0]; int i, c, k = 0; for (i = 0; i < nb_samples; i++) { double gain = fade_gain(curve, start + i * dir,range,silence,unity); for (c = 0; c < channels; c++, k++) d[k] = s[k] * gain; } }
static void fade_samples_flt (uint8_t **dst, uint8_t * const *src, int nb_samples, int channels, int dir, int64_t start, int64_t range, int curve, double silence, double unity) { float *d = (float *)dst[0]; const float *s = (float *)src[0]; int i, c, k = 0; for (i = 0; i < nb_samples; i++) { double gain = fade_gain(curve, start + i * dir,range,silence,unity); for (c = 0; c < channels; c++, k++) d[k] = s[k] * gain; } }
static void fade_samples_s16 (uint8_t **dst, uint8_t * const *src, int nb_samples, int channels, int dir, int64_t start, int64_t range, int curve, double silence, double unity) { int16_t *d = (int16_t *)dst[0]; const int16_t *s = (int16_t *)src[0]; int i, c, k = 0; for (i = 0; i < nb_samples; i++) { double gain = fade_gain(curve, start + i * dir,range,silence,unity); for (c = 0; c < channels; c++, k++) d[k] = s[k] * gain; } }
static void fade_samples_s32 (uint8_t **dst, uint8_t * const *src, int nb_samples, int channels, int dir, int64_t start, int64_t range, int curve, double silence, double unity) { int32_t *d = (int32_t *)dst[0]; const int32_t *s = (int32_t *)src[0]; int i, c, k = 0; for (i = 0; i < nb_samples; i++) { double gain = fade_gain(curve, start + i * dir,range,silence,unity); for (c = 0; c < channels; c++, k++) d[k] = s[k] * gain; } }
# 242 "/Users/davidchen/repo/ffmpeg/libavfilter/af_afade.c"
static void scale_samples_dblp(uint8_t **dst, uint8_t * const *src, int nb_samples, int channels, double gain) { int i, c; for (i = 0; i < nb_samples; i++) { for (c = 0; c < channels; c++) { double *d = (double *)dst[c]; const double *s = (double *)src[c]; d[i] = s[i] * gain; } } }
static void scale_samples_fltp(uint8_t **dst, uint8_t * const *src, int nb_samples, int channels, double gain) { int i, c; for (i = 0; i < nb_samples; i++) { for (c = 0; c < channels; c++) { float *d = (float *)dst[c]; const float *s = (float *)src[c]; d[i] = s[i] * gain; } } }
static void scale_samples_s16p(uint8_t **dst, uint8_t * const *src, int nb_samples, int channels, double gain) { int i, c; for (i = 0; i < nb_samples; i++) { for (c = 0; c < channels; c++) { int16_t *d = (int16_t *)dst[c]; const int16_t *s = (int16_t *)src[c]; d[i] = s[i] * gain; } } }
static void scale_samples_s32p(uint8_t **dst, uint8_t * const *src, int nb_samples, int channels, double gain) { int i, c; for (i = 0; i < nb_samples; i++) { for (c = 0; c < channels; c++) { int32_t *d = (int32_t *)dst[c]; const int32_t *s = (int32_t *)src[c]; d[i] = s[i] * gain; } } }

static void scale_samples_dbl (uint8_t **dst, uint8_t * const *src, int nb_samples, int channels, double gain){ double *d = (double *)dst[0]; const double *s = (double *)src[0]; int i, c, k = 0; for (i = 0; i < nb_samples; i++) { for (c = 0; c < channels; c++, k++) d[k] = s[k] * gain; } }
static void scale_samples_flt (uint8_t **dst, uint8_t * const *src, int nb_samples, int channels, double gain){ float *d = (float *)dst[0]; const float *s = (float *)src[0]; int i, c, k = 0; for (i = 0; i < nb_samples; i++) { for (c = 0; c < channels; c++, k++) d[k] = s[k] * gain; } }
static void scale_samples_s16 (uint8_t **dst, uint8_t * const *src, int nb_samples, int channels, double gain){ int16_t *d = (int16_t *)dst[0]; const int16_t *s = (int16_t *)src[0]; int i, c, k = 0; for (i = 0; i < nb_samples; i++) { for (c = 0; c < channels; c++, k++) d[k] = s[k] * gain; } }
static void scale_samples_s32 (uint8_t **dst, uint8_t * const *src, int nb_samples, int channels, double gain){ int32_t *d = (int32_t *)dst[0]; const int32_t *s = (int32_t *)src[0]; int i, c, k = 0; for (i = 0; i < nb_samples; i++) { for (c = 0; c < channels; c++, k++) d[k] = s[k] * gain; } }
...
static int config_output(AVFilterLink *outlink)
{
    ...
    if (s->duration)
        s->nb_samples = av_rescale(s->duration, outlink->sample_rate, 1000000);
    s->duration = 0;
    if (s->start_time)
        s->start_sample = av_rescale(s->start_time, outlink->sample_rate, 1000000);
    s->start_time = 0;
...
static const AVOption afade_options[] = {
    { "type", "set the fade direction", __builtin_offsetof(AudioFadeContext, type), AV_OPT_TYPE_INT, {.i64 = 0 }, 0, 1, 8|(1<<16)|(1<<15), "type" },
    { "t", "set the fade direction", __builtin_offsetof(AudioFadeContext, type), AV_OPT_TYPE_INT, {.i64 = 0 }, 0, 1, 8|(1<<16)|(1<<15), "type" },
    { "in", "fade-in", 0, AV_OPT_TYPE_CONST, {.i64 = 0 }, 0, 0, 8|(1<<16)|(1<<15), "type" },
    { "out", "fade-out", 0, AV_OPT_TYPE_CONST, {.i64 = 1 }, 0, 0, 8|(1<<16)|(1<<15), "type" },
    { "start_sample", "set number of first sample to start fading", __builtin_offsetof(AudioFadeContext, start_sample), AV_OPT_TYPE_INT64, {.i64 = 0 }, 0, 9223372036854775807LL, 8|(1<<16)|(1<<15) },
    { "ss", "set number of first sample to start fading", __builtin_offsetof(AudioFadeContext, start_sample), AV_OPT_TYPE_INT64, {.i64 = 0 }, 0, 9223372036854775807LL, 8|(1<<16)|(1<<15) },
    { "nb_samples", "set number of samples for fade duration", __builtin_offsetof(AudioFadeContext, nb_samples), AV_OPT_TYPE_INT64, {.i64 = 44100}, 1, 9223372036854775807LL, 8|(1<<16)|(1<<15) },
    { "ns", "set number of samples for fade duration", __builtin_offsetof(AudioFadeContext, nb_samples), AV_OPT_TYPE_INT64, {.i64 = 44100}, 1, 9223372036854775807LL, 8|(1<<16)|(1<<15) },
    { "start_time", "set time to start fading", __builtin_offsetof(AudioFadeContext, start_time), AV_OPT_TYPE_DURATION, {.i64 = 0 }, 0, 9223372036854775807LL, 8|(1<<16)|(1<<15) },
    { "st", "set time to start fading", __builtin_offsetof(AudioFadeContext, start_time), AV_OPT_TYPE_DURATION, {.i64 = 0 }, 0, 9223372036854775807LL, 8|(1<<16)|(1<<15) },
    { "duration", "set fade duration", __builtin_offsetof(AudioFadeContext, duration), AV_OPT_TYPE_DURATION, {.i64 = 0 }, 0, 9223372036854775807LL, 8|(1<<16)|(1<<15) },
    { "d", "set fade duration", __builtin_offsetof(AudioFadeContext, duration), AV_OPT_TYPE_DURATION, {.i64 = 0 }, 0, 9223372036854775807LL, 8|(1<<16)|(1<<15) },
    { "curve", "set fade curve type", __builtin_offsetof(AudioFadeContext, curve), AV_OPT_TYPE_INT, {.i64 = TRI }, NONE, NB_CURVES - 1, 8|(1<<16)|(1<<15), "curve" },
    { "c", "set fade curve type", __builtin_offsetof(AudioFadeContext, curve), AV_OPT_TYPE_INT, {.i64 = TRI }, NONE, NB_CURVES - 1, 8|(1<<16)|(1<<15), "curve" },
    { "nofade", "no fade; keep audio as-is", 0, AV_OPT_TYPE_CONST, {.i64 = NONE }, 0, 0, 8|(1<<16)|(1<<15), "curve" },
    { "tri", "linear slope", 0, AV_OPT_TYPE_CONST, {.i64 = TRI }, 0, 0, 8|(1<<16)|(1<<15), "curve" },
    { "qsin", "quarter of sine wave", 0, AV_OPT_TYPE_CONST, {.i64 = QSIN }, 0, 0, 8|(1<<16)|(1<<15), "curve" },
    { "esin", "exponential sine wave", 0, AV_OPT_TYPE_CONST, {.i64 = ESIN }, 0, 0, 8|(1<<16)|(1<<15), "curve" },
    { "hsin", "half of sine wave", 0, AV_OPT_TYPE_CONST, {.i64 = HSIN }, 0, 0, 8|(1<<16)|(1<<15), "curve" },
    { "log", "logarithmic", 0, AV_OPT_TYPE_CONST, {.i64 = LOG }, 0, 0, 8|(1<<16)|(1<<15), "curve" },
    { "ipar", "inverted parabola", 0, AV_OPT_TYPE_CONST, {.i64 = IPAR }, 0, 0, 8|(1<<16)|(1<<15), "curve" },
    { "qua", "quadratic", 0, AV_OPT_TYPE_CONST, {.i64 = QUA }, 0, 0, 8|(1<<16)|(1<<15), "curve" },
    { "cub", "cubic", 0, AV_OPT_TYPE_CONST, {.i64 = CUB }, 0, 0, 8|(1<<16)|(1<<15), "curve" },
    { "squ", "square root", 0, AV_OPT_TYPE_CONST, {.i64 = SQU }, 0, 0, 8|(1<<16)|(1<<15), "curve" },
    { "cbr", "cubic root", 0, AV_OPT_TYPE_CONST, {.i64 = CBR }, 0, 0, 8|(1<<16)|(1<<15), "curve" },
    { "par", "parabola", 0, AV_OPT_TYPE_CONST, {.i64 = PAR }, 0, 0, 8|(1<<16)|(1<<15), "curve" },
    { "exp", "exponential", 0, AV_OPT_TYPE_CONST, {.i64 = EXP }, 0, 0, 8|(1<<16)|(1<<15), "curve" },
    { "iqsin", "inverted quarter of sine wave", 0, AV_OPT_TYPE_CONST, {.i64 = IQSIN}, 0, 0, 8|(1<<16)|(1<<15), "curve" },
    { "ihsin", "inverted half of sine wave", 0, AV_OPT_TYPE_CONST, {.i64 = IHSIN}, 0, 0, 8|(1<<16)|(1<<15), "curve" },
    { "dese", "double-exponential seat", 0, AV_OPT_TYPE_CONST, {.i64 = DESE }, 0, 0, 8|(1<<16)|(1<<15), "curve" },
    { "desi", "double-exponential sigmoid", 0, AV_OPT_TYPE_CONST, {.i64 = DESI }, 0, 0, 8|(1<<16)|(1<<15), "curve" },
    { "losi", "logistic sigmoid", 0, AV_OPT_TYPE_CONST, {.i64 = LOSI }, 0, 0, 8|(1<<16)|(1<<15), "curve" },
    { "sinc", "sine cardinal function", 0, AV_OPT_TYPE_CONST, {.i64 = SINC }, 0, 0, 8|(1<<16)|(1<<15), "curve" },
    { "isinc", "inverted sine cardinal function", 0, AV_OPT_TYPE_CONST, {.i64 = ISINC}, 0, 0, 8|(1<<16)|(1<<15), "curve" },
    { "quat", "quartic", 0, AV_OPT_TYPE_CONST, {.i64 = QUAT }, 0, 0, 8|(1<<16)|(1<<15), "curve" },
    { "quatr", "quartic root", 0, AV_OPT_TYPE_CONST, {.i64 = QUATR}, 0, 0, 8|(1<<16)|(1<<15), "curve" },
    { "qsin2", "squared quarter of sine wave", 0, AV_OPT_TYPE_CONST, {.i64 = QSIN2}, 0, 0, 8|(1<<16)|(1<<15), "curve" },
    { "hsin2", "squared half of sine wave", 0, AV_OPT_TYPE_CONST, {.i64 = HSIN2}, 0, 0, 8|(1<<16)|(1<<15), "curve" },
    { "silence", "set the silence gain", __builtin_offsetof(AudioFadeContext, silence), AV_OPT_TYPE_DOUBLE, {.dbl = 0 }, 0, 1, 8|(1<<16)|(1<<15) },
    { "unity", "set the unity gain", __builtin_offsetof(AudioFadeContext, unity), AV_OPT_TYPE_DOUBLE, {.dbl = 1 }, 0, 1, 8|(1<<16)|(1<<15) },
    { ((void*)0) }
};

static const AVClass afade_class = { .class_name = "afade", .item_name = av_default_item_name, .option = afade_options, .version = ((58)<<16 | (36)<<8 | (100)), .category = AV_CLASS_CATEGORY_FILTER, };

static __attribute__((cold)) int init(AVFilterContext *ctx)
{
    AudioFadeContext *s = ctx->priv;

    if (9223372036854775807LL - s->nb_samples < s->start_sample)
        return (-(22));
...
const AVFilter ff_af_afade = {
    ...
    .init = init,
    .inputs = (avfilter_af_afade_inputs), .nb_inputs = (sizeof((avfilter_af_afade_inputs)) / sizeof(((avfilter_af_afade_inputs))[0])),
    .outputs = (avfilter_af_afade_outputs), .nb_outputs = (sizeof((avfilter_af_afade_outputs)) / sizeof(((avfilter_af_afade_outputs))[0])),
    .formats.samples_list = sample_fmts, .formats_state = FF_FILTER_FORMATS_SAMPLEFMTS_LIST,
...
static const AVOption acrossfade_options[] = {
    { "nb_samples", "set number of samples for cross fade duration", __builtin_offsetof(AudioFadeContext, nb_samples), AV_OPT_TYPE_INT, {.i64 = 44100}, 1, 2147483647/10, 8|(1<<16) },
    { "ns", "set number of samples for cross fade duration", __builtin_offsetof(AudioFadeContext, nb_samples), AV_OPT_TYPE_INT, {.i64 = 44100}, 1, 2147483647/10, 8|(1<<16) },
    { "duration", "set cross fade duration", __builtin_offsetof(AudioFadeContext, duration), AV_OPT_TYPE_DURATION, {.i64 = 0 }, 0, 60000000, 8|(1<<16) },
    { "d", "set cross fade duration", __builtin_offsetof(AudioFadeContext, duration), AV_OPT_TYPE_DURATION, {.i64 = 0 }, 0, 60000000, 8|(1<<16) },
    { "overlap", "overlap 1st stream end with 2nd stream start", __builtin_offsetof(AudioFadeContext, overlap), AV_OPT_TYPE_BOOL, {.i64 = 1 }, 0, 1, 8|(1<<16) },
    { "o", "overlap 1st stream end with 2nd stream start", __builtin_offsetof(AudioFadeContext, overlap), AV_OPT_TYPE_BOOL, {.i64 = 1 }, 0, 1, 8|(1<<16) },
    { "curve1", "set fade curve type for 1st stream", __builtin_offsetof(AudioFadeContext, curve), AV_OPT_TYPE_INT, {.i64 = TRI }, NONE, NB_CURVES - 1, 8|(1<<16), "curve" },
    { "c1", "set fade curve type for 1st stream", __builtin_offsetof(AudioFadeContext, curve), AV_OPT_TYPE_INT, {.i64 = TRI }, NONE, NB_CURVES - 1, 8|(1<<16), "curve" },
    { "nofade", "no fade; keep audio as-is", 0, AV_OPT_TYPE_CONST, {.i64 = NONE }, 0, 0, 8|(1<<16), "curve" },
    { "tri", "linear slope", 0, AV_OPT_TYPE_CONST, {.i64 = TRI }, 0, 0, 8|(1<<16), "curve" },
    { "qsin", "quarter of sine wave", 0, AV_OPT_TYPE_CONST, {.i64 = QSIN }, 0, 0, 8|(1<<16), "curve" },
    { "esin", "exponential sine wave", 0, AV_OPT_TYPE_CONST, {.i64 = ESIN }, 0, 0, 8|(1<<16), "curve" },
    { "hsin", "half of sine wave", 0, AV_OPT_TYPE_CONST, {.i64 = HSIN }, 0, 0, 8|(1<<16), "curve" },
    { "log", "logarithmic", 0, AV_OPT_TYPE_CONST, {.i64 = LOG }, 0, 0, 8|(1<<16), "curve" },
    { "ipar", "inverted parabola", 0, AV_OPT_TYPE_CONST, {.i64 = IPAR }, 0, 0, 8|(1<<16), "curve" },
    { "qua", "quadratic", 0, AV_OPT_TYPE_CONST, {.i64 = QUA }, 0, 0, 8|(1<<16), "curve" },
    { "cub", "cubic", 0, AV_OPT_TYPE_CONST, {.i64 = CUB }, 0, 0, 8|(1<<16), "curve" },
    { "squ", "square root", 0, AV_OPT_TYPE_CONST, {.i64 = SQU }, 0, 0, 8|(1<<16), "curve" },
    { "cbr", "cubic root", 0, AV_OPT_TYPE_CONST, {.i64 = CBR }, 0, 0, 8|(1<<16), "curve" },
    { "par", "parabola", 0, AV_OPT_TYPE_CONST, {.i64 = PAR }, 0, 0, 8|(1<<16), "curve" },
    { "exp", "exponential", 0, AV_OPT_TYPE_CONST, {.i64 = EXP }, 0, 0, 8|(1<<16), "curve" },
    { "iqsin", "inverted quarter of sine wave", 0, AV_OPT_TYPE_CONST, {.i64 = IQSIN}, 0, 0, 8|(1<<16), "curve" },
    { "ihsin", "inverted half of sine wave", 0, AV_OPT_TYPE_CONST, {.i64 = IHSIN}, 0, 0, 8|(1<<16), "curve" },
    { "dese", "double-exponential seat", 0, AV_OPT_TYPE_CONST, {.i64 = DESE }, 0, 0, 8|(1<<16), "curve" },
    { "desi", "double-exponential sigmoid", 0, AV_OPT_TYPE_CONST, {.i64 = DESI }, 0, 0, 8|(1<<16), "curve" },
    { "losi", "logistic sigmoid", 0, AV_OPT_TYPE_CONST, {.i64 = LOSI }, 0, 0, 8|(1<<16), "curve" },
    { "sinc", "sine cardinal function", 0, AV_OPT_TYPE_CONST, {.i64 = SINC }, 0, 0, 8|(1<<16), "curve" },
    { "isinc", "inverted sine cardinal function", 0, AV_OPT_TYPE_CONST, {.i64 = ISINC}, 0, 0, 8|(1<<16), "curve" },
    { "quat", "quartic", 0, AV_OPT_TYPE_CONST, {.i64 = QUAT }, 0, 0, 8|(1<<16), "curve" },
    { "quatr", "quartic root", 0, AV_OPT_TYPE_CONST, {.i64 = QUATR}, 0, 0, 8|(1<<16), "curve" },
    { "qsin2", "squared quarter of sine wave", 0, AV_OPT_TYPE_CONST, {.i64 = QSIN2}, 0, 0, 8|(1<<16), "curve" },
    { "hsin2", "squared half of sine wave", 0, AV_OPT_TYPE_CONST, {.i64 = HSIN2}, 0, 0, 8|(1<<16), "curve" },
    { "curve2", "set fade curve type for 2nd stream", __builtin_offsetof(AudioFadeContext, curve2), AV_OPT_TYPE_INT, {.i64 = TRI }, NONE, NB_CURVES - 1, 8|(1<<16), "curve" },
    { "c2", "set fade curve type for 2nd stream", __builtin_offsetof(AudioFadeContext, curve2), AV_OPT_TYPE_INT, {.i64 = TRI }, NONE, NB_CURVES - 1, 8|(1<<16), "curve" },
    { ((void*)0) }
};

static const AVClass acrossfade_class = { .class_name = "acrossfade", .item_name = av_default_item_name, .option = acrossfade_options, .version = ((58)<<16 | (36)<<8 | (100)), .category = AV_CLASS_CATEGORY_FILTER, };
# 534 "/Users/davidchen/repo/ffmpeg/libavfilter/af_afade.c"
static void crossfade_samples_dblp(uint8_t **dst, uint8_t * const *cf0, uint8_t * const *cf1, int nb_samples, int channels, int curve0, int curve1) { int i, c; for (i = 0; i < nb_samples; i++) { double gain0 = fade_gain(curve0, nb_samples - 1 - i, nb_samples,0.,1.); double gain1 = fade_gain(curve1, i, nb_samples, 0., 1.); for (c = 0; c < channels; c++) { double *d = (double *)dst[c]; const double *s0 = (double *)cf0[c]; const double *s1 = (double *)cf1[c]; d[i] = s0[i] * gain0 + s1[i] * gain1; } } }
static void crossfade_samples_fltp(uint8_t **dst, uint8_t * const *cf0, uint8_t * const *cf1, int nb_samples, int channels, int curve0, int curve1) { int i, c; for (i = 0; i < nb_samples; i++) { double gain0 = fade_gain(curve0, nb_samples - 1 - i, nb_samples,0.,1.); double gain1 = fade_gain(curve1, i, nb_samples, 0., 1.); for (c = 0; c < channels; c++) { float *d = (float *)dst[c]; const float *s0 = (float *)cf0[c]; const float *s1 = (float *)cf1[c]; d[i] = s0[i] * gain0 + s1[i] * gain1; } } }
static void crossfade_samples_s16p(uint8_t **dst, uint8_t * const *cf0, uint8_t * const *cf1, int nb_samples, int channels, int curve0, int curve1) { int i, c; for (i = 0; i < nb_samples; i++) { double gain0 = fade_gain(curve0, nb_samples - 1 - i, nb_samples,0.,1.); double gain1 = fade_gain(curve1, i, nb_samples, 0., 1.); for (c = 0; c < channels; c++) { int16_t *d = (int16_t *)dst[c]; const int16_t *s0 = (int16_t *)cf0[c]; const int16_t *s1 = (int16_t *)cf1[c]; d[i] = s0[i] * gain0 + s1[i] * gain1; } } }
static void crossfade_samples_s32p(uint8_t **dst, uint8_t * const *cf0, uint8_t * const *cf1, int nb_samples, int channels, int curve0, int curve1) { int i, c; for (i = 0; i < nb_samples; i++) { double gain0 = fade_gain(curve0, nb_samples - 1 - i, nb_samples,0.,1.); double gain1 = fade_gain(curve1, i, nb_samples, 0., 1.); for (c = 0; c < channels; c++) { int32_t *d = (int32_t *)dst[c]; const int32_t *s0 = (int32_t *)cf0[c]; const int32_t *s1 = (int32_t *)cf1[c]; d[i] = s0[i] * gain0 + s1[i] * gain1; } } }

static void crossfade_samples_dbl (uint8_t **dst, uint8_t * const *cf0, uint8_t * const *cf1, int nb_samples, int channels, int curve0, int curve1) { double *d = (double *)dst[0]; const double *s0 = (double *)cf0[0]; const double *s1 = (double *)cf1[0]; int i, c, k = 0; for (i = 0; i < nb_samples; i++) { double gain0 = fade_gain(curve0, nb_samples - 1-i,nb_samples,0.,1.); double gain1 = fade_gain(curve1, i, nb_samples, 0., 1.); for (c = 0; c < channels; c++, k++) d[k] = s0[k] * gain0 + s1[k] * gain1; } }
static void crossfade_samples_flt (uint8_t **dst, uint8_t * const *cf0, uint8_t * const *cf1, int nb_samples, int channels, int curve0, int curve1) { float *d = (float *)dst[0]; const float *s0 = (float *)cf0[0]; const float *s1 = (float *)cf1[0]; int i, c, k = 0; for (i = 0; i < nb_samples; i++) { double gain0 = fade_gain(curve0, nb_samples - 1-i,nb_samples,0.,1.); double gain1 = fade_gain(curve1, i, nb_samples, 0., 1.); for (c = 0; c < channels; c++, k++) d[k] = s0[k] * gain0 + s1[k] * gain1; } }
static void crossfade_samples_s16 (uint8_t **dst, uint8_t * const *cf0, uint8_t * const *cf1, int nb_samples, int channels, int curve0, int curve1) { int16_t *d = (int16_t *)dst[0]; const int16_t *s0 = (int16_t *)cf0[0]; const int16_t *s1 = (int16_t *)cf1[0]; int i, c, k = 0; for (i = 0; i < nb_samples; i++) { double gain0 = fade_gain(curve0, nb_samples - 1-i,nb_samples,0.,1.); double gain1 = fade_gain(curve1, i, nb_samples, 0., 1.); for (c = 0; c < channels; c++, k++) d[k] = s0[k] * gain0 + s1[k] * gain1; } }
static void crossfade_samples_s32 (uint8_t **dst, uint8_t * const *cf0, uint8_t * const *cf1, int nb_samples, int channels, int curve0, int curve1) { int32_t *d = (int32_t *)dst[0]; const int32_t *s0 = (int32_t *)cf0[0]; const int32_t *s1 = (int32_t *)cf1[0]; int i, c, k = 0; for (i = 0; i < nb_samples; i++) { double gain0 = fade_gain(curve0, nb_samples - 1-i,nb_samples,0.,1.); double gain1 = fade_gain(curve1, i, nb_samples, 0., 1.); for (c = 0; c < channels; c++, k++) d[k] = s0[k] * gain0 + s1[k] * gain1; } }
...
static int activate(AVFilterContext *ctx)
{
    ...
    AVFrame *in = ((void*)0), *out, *cf[2] = { ((void*)0) };
    ...
    if (nb_samples > s->nb_samples) {
        ...
        if (!s->status[0] && check_input(ctx->inputs[0]))
            s->status[0] = (-(int)(('E') | (('O') << 8) | (('F') << 16) | ((unsigned)(' ') << 24)));
        s->passthrough = !s->status[0];
        if (check_input(ctx->inputs[1])) {
            s->status[1] = (-(int)(('E') | (('O') << 8) | (('F') << 16) | ((unsigned)(' ') << 24)));
            ff_outlink_set_status(outlink, (-(int)(('E') | (('O') << 8) | (('F') << 16) | ((unsigned)(' ') << 24))), ((int64_t)(0x8000000000000000ULL)));
            return 0;
        }
        if (!s->status[0])
...
const AVFilter ff_af_acrossfade = {
    ...
    .inputs = (avfilter_af_acrossfade_inputs), .nb_inputs = (sizeof((avfilter_af_acrossfade_inputs)) / sizeof(((avfilter_af_acrossfade_inputs))[0])),
    .outputs = (avfilter_af_acrossfade_outputs), .nb_outputs = (sizeof((avfilter_af_acrossfade_outputs)) / sizeof(((avfilter_af_acrossfade_outputs))[0])),
    .formats.samples_list = sample_fmts, .formats_state = FF_FILTER_FORMATS_SAMPLEFMTS_LIST,
};
