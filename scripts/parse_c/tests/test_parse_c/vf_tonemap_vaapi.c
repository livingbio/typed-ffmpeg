# 1 "/Users/davidchen/repo/ffmpeg/libavfilter/vf_tonemap_vaapi.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 418 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/Users/davidchen/repo/ffmpeg/libavfilter/vf_tonemap_vaapi.c" 2
# 18 "/Users/davidchen/repo/ffmpeg/libavfilter/vf_tonemap_vaapi.c"
...
# 19 "/Users/davidchen/repo/ffmpeg/libavfilter/vf_tonemap_vaapi.c" 2
...
int av_opt_serialize(void *obj, int opt_flags, int flags, char **buffer,
                     const char key_val_sep, const char pairs_sep);
# 21 "/Users/davidchen/repo/ffmpeg/libavfilter/vf_tonemap_vaapi.c" 2
...
# 22 "/Users/davidchen/repo/ffmpeg/libavfilter/vf_tonemap_vaapi.c" 2
# 1 "./libavutil/mastering_display_metadata.h" 1
# 24 "./libavutil/mastering_display_metadata.h"
...
# 25 "./libavutil/mastering_display_metadata.h" 2
# 38 "./libavutil/mastering_display_metadata.h"
...
# 78 "./libavutil/mastering_display_metadata.h"
AVMasteringDisplayMetadata *av_mastering_display_metadata_alloc(void);
# 87 "./libavutil/mastering_display_metadata.h"
AVMasteringDisplayMetadata *av_mastering_display_metadata_create_side_data(AVFrame *frame);
# 98 "./libavutil/mastering_display_metadata.h"
...
# 117 "./libavutil/mastering_display_metadata.h"
AVContentLightMetadata *av_content_light_metadata_alloc(size_t *size);
# 126 "./libavutil/mastering_display_metadata.h"
AVContentLightMetadata *av_content_light_metadata_create_side_data(AVFrame *frame);
# 23 "/Users/davidchen/repo/ffmpeg/libavfilter/vf_tonemap_vaapi.c" 2
...
# 25 "/Users/davidchen/repo/ffmpeg/libavfilter/vf_tonemap_vaapi.c" 2
...
# 26 "/Users/davidchen/repo/ffmpeg/libavfilter/vf_tonemap_vaapi.c" 2
# 1 "/Users/davidchen/repo/ffmpeg/libavfilter/vaapi_vpp.h" 1
# 25 "/Users/davidchen/repo/ffmpeg/libavfilter/vaapi_vpp.h"
...
int av_hwframe_ctx_create_derived(AVBufferRef **derived_frame_ctx,
                                  ...
                                  int flags);
# 26 "/Users/davidchen/repo/ffmpeg/libavfilter/vaapi_vpp.h" 2
...
int ff_vaapi_vpp_render_pictures(AVFilterContext *avctx,
                                 ...
                                 int count,
                                 AVFrame *output_frame);
# 27 "/Users/davidchen/repo/ffmpeg/libavfilter/vf_tonemap_vaapi.c" 2
...
# 28 "/Users/davidchen/repo/ffmpeg/libavfilter/vf_tonemap_vaapi.c" 2
...
static int tonemap_vaapi_save_metadata(AVFilterContext *avctx, AVFrame *input_frame)
{
    ...
    if (input_frame->color_trc != AVCOL_TRC_SMPTE2084) {
        av_log(avctx, 24, "Only support HDR10 as input for vaapi tone-mapping\n");
    }
    ...
    if (ctx->src_display) {
        ...
        if (!hdr_meta) {
            av_log(avctx, 16, "No mastering display data\n");
            return (-(22));
        }
        ...
        if (hdr_meta->has_luminance) {
            ...
            ctx->in_metadata.min_display_mastering_luminance =
                ((lrint(luma_den * av_q2d(hdr_meta->min_luminance))) > (ctx->in_metadata.max_display_mastering_luminance) ? (ctx->in_metadata.max_display_mastering_luminance) : (lrint(luma_den * av_q2d(hdr_meta->min_luminance))));
        ...
        if (hdr_meta->has_primaries) {
            ...
            for (i = 0; i < 3; i++) {
                ...
                ctx->in_metadata.display_primaries_x[i] =
                    ((lrint(chroma_den * av_q2d(hdr_meta->display_primaries[j][0]))) > (chroma_den) ? (chroma_den) : (lrint(chroma_den * av_q2d(hdr_meta->display_primaries[j][0]))));


                ctx->in_metadata.display_primaries_y[i] =
                    ((lrint(chroma_den * av_q2d(hdr_meta->display_primaries[j][1]))) > (chroma_den) ? (chroma_den) : (lrint(chroma_den * av_q2d(hdr_meta->display_primaries[j][1]))));


            }

            ctx->in_metadata.white_point_x =
                ((lrint(chroma_den * av_q2d(hdr_meta->white_point[0]))) > (chroma_den) ? (chroma_den) : (lrint(chroma_den * av_q2d(hdr_meta->white_point[0]))));

            ctx->in_metadata.white_point_y =
                ((lrint(chroma_den * av_q2d(hdr_meta->white_point[1]))) > (chroma_den) ? (chroma_den) : (lrint(chroma_den * av_q2d(hdr_meta->white_point[1]))));
        ...
        av_log(avctx, 16, "No mastering display data from input\n");
        return (-(22));
    }
    ...
    if (ctx->src_light) {
        ...
        if (!light_meta) {
            av_log(avctx, 16, "No light metadata\n");
            return (-(22));
        }
        ...
        av_log(avctx, 48, "No content light level from input\n");
    }
    return 0;
}
...
static int tonemap_vaapi_set_filter_params(AVFilterContext *avctx, AVFrame *input_frame)
{
    ...
    if (vas != VA_STATUS_SUCCESS) {
        av_log(avctx, 16, "Failed to map "
               "buffer (%d): %d (%s).\n",
               vpp_ctx->filter_buffers[0], vas, vaErrorStr(vas));
        return (-(5));
    }

    __builtin___memcpy_chk (hdrtm_param->data.metadata, &ctx->in_metadata, sizeof(VAHdrMetaDataHDR10), __builtin_object_size (hdrtm_param->data.metadata, 0));

    vas = vaUnmapBuffer(vpp_ctx->hwctx->display, vpp_ctx->filter_buffers[0]);
    if (vas != VA_STATUS_SUCCESS) {
        av_log(avctx, 16, "Failed to unmap output buffers: "
               "%d (%s).\n", vas, vaErrorStr(vas));
        return (-(5));
    }
...
static int tonemap_vaapi_build_filter_params(AVFilterContext *avctx)
{
    ...
    __builtin___memset_chk (&hdrtm_param, 0, sizeof(hdrtm_param), __builtin_object_size (&hdrtm_param, 0));
    __builtin___memset_chk (&ctx->in_metadata, 0, sizeof(ctx->in_metadata), __builtin_object_size (&ctx->in_metadata, 0));
    ...
    if (vas != VA_STATUS_SUCCESS) {
        av_log(avctx, 16, "Failed to query HDR caps "
               "context: %d (%s).\n", vas, vaErrorStr(vas));
        return (-(5));
    }
    ...
    if (i >= num_query_caps) {
        av_log(avctx, 16, "VAAPI driver doesn't support HDR\n");
        return (-(22));
    }
...
static int tonemap_vaapi_filter_frame(AVFilterLink *inlink, AVFrame *input_frame)
{
    ...
    VAAPIVPPContext *vpp_ctx = avctx->priv;
    HDRVAAPIContext *ctx = avctx->priv;
    AVFrame *output_frame = ((void*)0);
    VASurfaceID input_surface, output_surface;

    VAProcPipelineParameterBuffer params;
    int err;

    av_log(avctx, 48, "Filter input: %s, %ux%u (%""ll" "d"").\n",
    ...
    av_log(avctx, 48, "Using surface %#x for tonemap vpp input.\n",
           input_surface);
    ...
    av_log(avctx, 48, "Using surface %#x for tonemap vpp output.\n",
           output_surface);
    ...
    av_log(avctx, 48, "Filter output: %s, %ux%u (%""ll" "d"").\n",
...
static __attribute__((cold)) int tonemap_vaapi_init(AVFilterContext *avctx)
{
    VAAPIVPPContext *vpp_ctx = avctx->priv;
    HDRVAAPIContext *ctx = avctx->priv;
    ...
    if (ctx->output_format_string) {
        ...
        case AV_PIX_FMT_P010LE:
            break;
        default:
            av_log(avctx, 16, "Invalid output format.\n");
            return (-(22));
        }
    } else {
        vpp_ctx->output_format = AV_PIX_FMT_NV12;
        av_log(avctx, 24, "Output format not set, use default format NV12\n");
    }
# 360 "/Users/davidchen/repo/ffmpeg/libavfilter/vf_tonemap_vaapi.c"
    do { if (ctx->color_primaries_string) { int var = av_color_primaries_from_name(ctx->color_primaries_string); if (var < 0) { av_log(avctx, 16, "Invalid %s.\n", "color_primaries"); return (-(22)); } ctx->color_primaries = var; } else { ctx->color_primaries = AVCOL_PRI_UNSPECIFIED; } } while (0);
    do { if (ctx->color_transfer_string) { int var = av_color_transfer_from_name(ctx->color_transfer_string); if (var < 0) { av_log(avctx, 16, "Invalid %s.\n", "color_transfer"); return (-(22)); } ctx->color_transfer = var; } else { ctx->color_transfer = AVCOL_TRC_UNSPECIFIED; } } while (0);
    do { if (ctx->color_matrix_string) { int var = av_color_space_from_name(ctx->color_matrix_string); if (var < 0) { av_log(avctx, 16, "Invalid %s.\n", "color_matrix"); return (-(22)); } ctx->color_matrix = var; } else { ctx->color_matrix = AVCOL_SPC_UNSPECIFIED; } } while (0);
...
static const AVOption tonemap_vaapi_options[] = {
    { "format", "Output pixel format set", __builtin_offsetof(HDRVAAPIContext, output_format_string), AV_OPT_TYPE_STRING, .flags = (16 | (1<<16)), "format" },
    { "matrix", "Output color matrix coefficient set",
      __builtin_offsetof(HDRVAAPIContext, color_matrix_string), AV_OPT_TYPE_STRING,
      { .str = ((void*)0) }, .flags = (16 | (1<<16)), "matrix" },
    { "m", "Output color matrix coefficient set",
      __builtin_offsetof(HDRVAAPIContext, color_matrix_string), AV_OPT_TYPE_STRING,
      { .str = ((void*)0) }, .flags = (16 | (1<<16)), "matrix" },
    { "primaries", "Output color primaries set",
      __builtin_offsetof(HDRVAAPIContext, color_primaries_string), AV_OPT_TYPE_STRING,
      { .str = ((void*)0) }, .flags = (16 | (1<<16)), "primaries" },
    { "p", "Output color primaries set",
      __builtin_offsetof(HDRVAAPIContext, color_primaries_string), AV_OPT_TYPE_STRING,
      { .str = ((void*)0) }, .flags = (16 | (1<<16)), "primaries" },
    { "transfer", "Output color transfer characteristics set",
      __builtin_offsetof(HDRVAAPIContext, color_transfer_string), AV_OPT_TYPE_STRING,
      { .str = ((void*)0) }, .flags = (16 | (1<<16)), "transfer" },
...
static const AVClass tonemap_vaapi_class = { .class_name = "tonemap_vaapi", .item_name = av_default_item_name, .option = tonemap_vaapi_options, .version = ((58)<<16 | (36)<<8 | (100)), .category = AV_CLASS_CATEGORY_FILTER, };
...
const AVFilter ff_vf_tonemap_vaapi = {
    ...
    .inputs = (tonemap_vaapi_inputs), .nb_inputs = (sizeof((tonemap_vaapi_inputs)) / sizeof(((tonemap_vaapi_inputs))[0])),
    .outputs = (tonemap_vaapi_outputs), .nb_outputs = (sizeof((tonemap_vaapi_outputs)) / sizeof(((tonemap_vaapi_outputs))[0])),
    .formats.query_func = &ff_vaapi_vpp_query_formats, .formats_state = FF_FILTER_FORMATS_QUERY_FUNC,
