...
Args:
    min_val: Set the minimal luminance value. Default is 16.
    timeline_options: Timeline options
    extra_options: Extra options for the filter
...
Args:
    ...
    dnn_backend: Specify which DNN backend to use for model loading and execution. This option accepts the following values: @end table Default value is native.
    model: Set path to model file specifying network architecture and its parameters. Note that different backends use different file formats. TensorFlow and native backend can load files for only its format.
...
Args:
    dnn_backend: Specify which DNN backend to use for model loading and execution. This option accepts the following values: @end table Default value is native.
    model: Set path to model file specifying network architecture and its parameters. Note that different backends use different file formats. TensorFlow, OpenVINO and native backend can load files for only its format. Native model file (.model) can be generated from TensorFlow model file (.pb) by using tools/python/convert.py
    input: Set the input name of the dnn network.
    output: Set the output name of the dnn network.
...
Args:
    ...
    ecost: Specify the edge cost for edge matching. Default value is 1.0. Allowed range is from 0 to 9.
    mcost: Specify the middle cost for edge matching. Default value is 0.5. Allowed range is from 0 to 1.
    dcost: Specify the distance cost for edge matching. Default value is 0.5. Allowed range is from 0 to 1.
...
Args:
    ...
    window: set window function (from 0 to 19) (default hann)
    timeline_options: Timeline options
    extra_options: Extra options for the filter
...
Args:
    ...
    threshold: Detection threshold, default is 0.5.
...
Args:
    lum_expr: Set the luminance expression.
...
Args:
    ...
    mode: Set output mode, can be fulll or compact. In compact mode only filters with some queued frames have displayed stats.
...
Args:
    ...
    use_alpha: Create a palette of colors with alpha components. Setting this, will automatically disable 'reserve_transparent'.
    extra_options: Extra options for the filter
...
Args:
    ...
    use_alpha: Apply the palette by taking alpha values into account. Only useful with palettes that are containing multiple colors with alpha components. Setting this will automatically disable 'alpha_treshold'.
    debug_kdtree: save Graphviz graph of the kdtree in specified file
    color_search: set reverse colormap color search method (from 0 to 2) (default nns_iterative)
    mean_err: compute and print mean error (default false)
    debug_accuracy: test color search accuracy (default false)
    extra_options: Extra options for the filter
...
Args:
    ...
    param0: Scaler param 0 (from INT_MIN to INT_MAX) (default 123456)
    param1: Scaler param 1 (from INT_MIN to INT_MAX) (default 123456)
...
Args:
    ...
    model: Set path to model file specifying network architecture and its parameters. Note that different backends use different file formats. TensorFlow backend can load files for both formats, while native backend can load files for only its format.
    input: input name of the model (default "x")
    output: output name of the model (default "y")
    extra_options: Extra options for the filter
...
Args:
    ...
    components: Set which color components to display. Default is 1, which means only luminance or red color component if input is in RGB colorspace. If is set for example to 7 it will display all 3 (if) available color components.
    envelope: @end table
    filter: @end table
