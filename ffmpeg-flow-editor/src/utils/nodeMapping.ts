...
export const removeEdgeFromMapping = (edgeId: string) => {
  ...
  if (stream) {
    // Extract target node ID and index from edge ID
    const [, , , targetId, targetIndex] = edgeId.split('-');
    const targetNode = nodeMapping.nodeMap.get(targetId);

    // Clear the input at the specified index
    if (targetNode instanceof FilterNode || targetNode instanceof OutputNode) {
      const index = parseInt(targetIndex);
      if (index < targetNode.inputs.length) {
        targetNode.inputs[index] = null;
      }
