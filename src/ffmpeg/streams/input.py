...
class _InputStream(FilterableStream):
    node: InputNode
    index: int | None = None


@dataclass(frozen=True, kw_only=True)
class VideoIStream(VideoFilter, _InputStream):
    @override
...
@dataclass(frozen=True, kw_only=True)
class AudioIStream(AudioFilter, _InputStream):
    @override
    def label(self, context: _DAGContext) -> str:
...
@dataclass(frozen=True, kw_only=True)
class AVStream(VideoFilter, AudioFilter, _InputStream):
    @property
