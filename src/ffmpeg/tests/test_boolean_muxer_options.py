...
def test_hls_muxer_strftime_boolean_option(snapshot: SnapshotAssertion) -> None:
    ...
    out = input1.output(
        filename="out.m3u8",
        f="hls",
        muxer_options=formats.muxers.hls(strftime=True)
    )
...
def test_hls_muxer_strftime_false_boolean_option(snapshot: SnapshotAssertion) -> None:
    ...
    out = input1.output(
        filename="out.m3u8",
        f="hls",
        muxer_options=formats.muxers.hls(strftime=False)
    )
    assert snapshot(extension_class=JSONSnapshotExtension) == (out.compile())


def test_segment_muxer_reset_timestamps_boolean_option(snapshot: SnapshotAssertion) -> None:
    """
