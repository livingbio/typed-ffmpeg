...
def test_validate_reuse_stream(snapshot: SnapshotAssertion) -> None:
    ...
    rev = input1.reverse()
    context = DAGContext.build(concat(rev.trim(), rev.trim()).video(0).output(filename="tmp.mp4").node)
...
def test_validate_not_utilize_split(snapshot: SnapshotAssertion) -> None:
    ...
    context = DAGContext.build(input1.split(outputs=2).video(0).output(filename="tmp.mp4").node)
    with pytest.raises(AssertionError) as e:
...
def test_reduntant_split_outputs_1(snapshot: SnapshotAssertion) -> None:
    ...
    context = DAGContext.build(input1.split(outputs=1).video(0).output(filename="tmp.mp4").node)
    with pytest.raises(AssertionError) as e:
        _validate_not_utilize_split(context)
...
def test_reduntant_split_duplicate(snapshot: SnapshotAssertion) -> None:
    ...
    context = DAGContext.build(concat(s00, s01, s1, n=3).video(0).output(filename="tmp.mp4").node)
    with pytest.raises(AssertionError) as e:
        _validate_not_utilize_split(context)
