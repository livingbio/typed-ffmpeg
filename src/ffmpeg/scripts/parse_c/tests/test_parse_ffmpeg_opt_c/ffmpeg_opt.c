...
# 69 "/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include/sys/time.h" 3 4
...
# 70 "/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include/sys/time.h" 2 3 4
...
# 77 "/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include/sys/time.h" 2 3 4
...
# 78 "/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include/sys/time.h" 2 3 4
...
# 79 "/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include/sys/time.h" 2 3 4


# 1 "/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include/sys/_types/_timeval64.h" 1 3 4
# 34 "/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include/sys/_types/_timeval64.h" 3 4
struct timeval64 {
 __int64_t tv_sec;
 __int64_t tv_usec;
};
# 82 "/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include/sys/time.h" 2 3 4
...
# 87 "/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include/sys/time.h" 2 3 4
...
# 111 "/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include/sys/time.h" 3 4
...
# 116 "/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include/sys/time.h" 2 3 4



# 1 "/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include/sys/_types/_fd_copy.h" 1 3 4
# 120 "/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include/sys/time.h" 2 3 4
# 130 "/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include/sys/time.h" 3 4
struct timezone {
 int tz_minuteswest;
 int tz_dsttime;
};
# 173 "/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include/sys/time.h" 3 4
struct clockinfo {
 int hz;
 int tick;
 int tickadj;
...
# 186 "/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include/sys/time.h" 2 3 4
...
;
# 203 "/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include/sys/time.h" 2 3 4
...
# 68 "/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include/sys/resource.h" 2 3 4
# 83 "/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include/sys/resource.h" 3 4
...
# 84 "/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include/sys/resource.h" 2 3 4
...
# 154 "/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include/sys/signal.h" 2 3 4
...
{
    __builtin___memset_chk (o, 0, sizeof(*o), __builtin_object_size (o, 0));

    o->stop_time = 9223372036854775807LL;
    o->mux_max_delay = 0.7;
    o->start_time = ((int64_t)(0x8000000000000000ULL));
    o->start_time_eof = ((int64_t)(0x8000000000000000ULL));
    o->recording_time = 9223372036854775807LL;
...
AVDictionary *strip_specifiers(const AVDictionary *dict)
{
    const AVDictionaryEntry *e = ((void*)0);
...
int parse_and_set_vsync(const char *arg, int *vsync_var, int file_idx, int st_idx, int is_global)
{
    if (!av_strcasecmp(arg, "cfr")) *vsync_var = VSYNC_CFR;
        ...
        av_log(((void*)0), 24, "-vsync/fps_mode drop is deprecated\n");
        *vsync_var = VSYNC_DROP;
    }

    else if (!is_global && !av_strcasecmp(arg, "auto")) *vsync_var = VSYNC_AUTO;
    else if (!is_global) {
        av_log(((void*)0), 8, "Invalid value %s specified for fps_mode of #%d:%d.\n", arg, file_idx, st_idx);
        return (-(22));
    }
...
static void correct_input_start_times(void)
{
    for (int i = 0; i < nb_input_files; i++) {
        ...
        int64_t new_start_time = 9223372036854775807LL, diff, abs_start_seek;

        ifile->start_time_effective = is->start_time;

        if (is->start_time == ((int64_t)(0x8000000000000000ULL)) ||
            !(is->iformat->flags & 0x0200))
            continue;

        for (int j = 0; j < is->nb_streams; j++) {
            AVStream *st = is->streams[j];
            if(st->discard == AVDISCARD_ALL || st->start_time == ((int64_t)(0x8000000000000000ULL)))
                continue;
        ...
        if (diff) {
            av_log(((void*)0), 40, "Correcting start time of Input #%d by %""ll" "d"" us.\n", i, diff);
            ifile->start_time_effective = new_start_time;
            if (copy_ts && start_at_zero)
                ifile->ts_offset = -new_start_time;
            else if (!copy_ts) {
                abs_start_seek = is->start_time + (ifile->start_time != ((int64_t)(0x8000000000000000ULL))) ? ifile->start_time : 0;
...
static int apply_sync_offsets(void)
{
    for (int i = 0; i < nb_input_files; i++) {
        ...
        if (self->input_sync_ref >= nb_input_files || self->input_sync_ref < -1) {
            av_log(((void*)0), 8, "-isync for input %d references non-existent input %d.\n", i, self->input_sync_ref);
            return (-(22));
        }

        if (copy_ts && !start_at_zero) {
            av_log(((void*)0), 8, "Use of -isync requires that start_at_zero be set if copyts is set.\n");
            return (-(22));
        }

        ref = input_files[self->input_sync_ref];
        if (ref->input_sync_ref != -1 && ref->input_sync_ref != self->input_sync_ref) {
            av_log(((void*)0), 16, "-isync for input %d references a resynced input %d. Sync not set.\n", i, self->input_sync_ref);
            continue;
        }

        if (self->ctx->start_time_realtime != ((int64_t)(0x8000000000000000ULL)) && ref->ctx->start_time_realtime != ((int64_t)(0x8000000000000000ULL))) {
            self_start_time = self->ctx->start_time_realtime;
            ref_start_time = ref->ctx->start_time_realtime;
        } else if (self->start_time_effective != ((int64_t)(0x8000000000000000ULL)) && ref->start_time_effective != ((int64_t)(0x8000000000000000ULL))) {
        ...
        if (start_times_set) {
            self_seek_start = self->start_time == ((int64_t)(0x8000000000000000ULL)) ? 0 : self->start_time;
            ref_seek_start = ref->start_time == ((int64_t)(0x8000000000000000ULL)) ? 0 : ref->start_time;
            ...
            av_log(((void*)0), 32, "Adjusted ts offset for Input #%d by %""ll" "d"" us to sync with Input #%d.\n", i, adjustment, self->input_sync_ref);
        } else {
            av_log(((void*)0), 32, "Unable to identify start times for Inputs #%d and %d both. No sync adjustment made.\n", i, self->input_sync_ref);
        }
    }
...
static int opt_abort_on(void *optctx, const char *opt, const char *arg)
{
    static const AVOption opts[] = {
        { "abort_on" , ((void*)0), 0, AV_OPT_TYPE_FLAGS, { .i64 = 0 }, (-9223372036854775807LL -1), 9223372036854775807LL, .unit = "flags" },
        { "empty_output" , ((void*)0), 0, AV_OPT_TYPE_CONST, { .i64 = (1 << 0) }, .unit = "flags" },
        { "empty_output_stream", ((void*)0), 0, AV_OPT_TYPE_CONST, { .i64 = (1 << 1) }, .unit = "flags" },
        { ((void*)0) },
    };
...
static int opt_stats_period(void *optctx, const char *opt, const char *arg)
{
    ...
    if (user_stats_period <= 0) {
        av_log(((void*)0), 16, "stats_period %s must be positive.\n", arg);
        return (-(22));
    }

    stats_period = user_stats_period;
    av_log(((void*)0), 32, "ffmpeg stats and -progress period set to %s.\n", arg);
...
static int opt_map(void *optctx, const char *opt, const char *arg)
{
...

    {
        ...
        if (sync) {
            *sync = 0;
            av_log(((void*)0), 24, "Specifying a sync stream is deprecated and has no effect\n");
        }
    }
    ...
    if (map[0] == '[') {
        ...
        if (!m->linklabel) {
            av_log(((void*)0), 16, "Invalid output link label: %s.\n", map);
            ret = (-(22));
            goto fail;
        }
    } else {
        ...
        if (file_idx >= nb_input_files || file_idx < 0) {
            av_log(((void*)0), 8, "Invalid input file index: %d.\n", file_idx);
            ret = (-(22));
            goto fail;
        }
        if (negative)
    ...
    if (!m) {
        if (allow_unused) {
            av_log(((void*)0), 40, "Stream map '%s' matches no streams; ignoring.\n", arg);
        } else if (disabled) {
            av_log(((void*)0), 8, "Stream map '%s' matches disabled streams.\n"
                                       "To ignore this, add a trailing '?' to the map.\n", arg);
            ret = (-(22));
            goto fail;
        } else {
            av_log(((void*)0), 8, "Stream map '%s' matches no streams.\n"
                                       "To ignore this, add a trailing '?' to the map.\n", arg);
            ret = (-(22));
            goto fail;
        }
...
static int opt_map_channel(void *optctx, const char *opt, const char *arg)
{
    ...
    ret = grow_array((void**)&o->audio_channel_maps, sizeof(*o->audio_channel_maps), &o->nb_audio_channel_maps, o->nb_audio_channel_maps + 1);
    if (ret < 0)
        goto end;
    ...
    if (n != 3 && n != 5) {
        av_log(((void*)0), 8, "Syntax error, mapchan usage: "
    ...
    if (m->file_idx < 0 || m->file_idx >= nb_input_files) {
        av_log(((void*)0), 8, "mapchan: invalid input file index: %d\n",
               m->file_idx);
        goto fail;
    }
    if (m->stream_idx < 0 ||
        m->stream_idx >= input_files[m->file_idx]->nb_streams) {
        av_log(((void*)0), 8, "mapchan: invalid input file stream index #%d.%d\n",
               m->file_idx, m->stream_idx);
        goto fail;
    }
    st = input_files[m->file_idx]->ctx->streams[m->stream_idx];
    if (st->codecpar->codec_type != AVMEDIA_TYPE_AUDIO) {
        av_log(((void*)0), 8, "mapchan: stream #%d.%d is not an audio stream.\n",
               m->file_idx, m->stream_idx);
        goto fail;
    }
    ...
    if (m->channel_idx < 0 || m->channel_idx >= st->codecpar->ch_layout.nb_channels ||
        ...
        if (allow_unused) {
            av_log(((void*)0), 40, "mapchan: invalid audio channel #%d.%d.%d\n",
                    m->file_idx, m->stream_idx, m->channel_idx);
        } else {
            av_log(((void*)0), 8, "mapchan: invalid audio channel #%d.%d.%d\n"
...
{
    if (filter_hw_device) {
        av_log(((void*)0), 16, "Only one filter device can be used.\n");
        return (-(22));
    }
    filter_hw_device = hw_device_get_by_name(arg);
    if (!filter_hw_device) {
        av_log(((void*)0), 16, "Invalid filter device %s.\n", arg);
        return (-(22));
    }
    return 0;
}
...
static int opt_recording_timestamp(void *optctx, const char *opt, const char *arg)
{
    ...
    av_log(((void*)0), 24, "%s is deprecated, set the 'creation_time' metadata "
                                 "tag instead.\n", opt);
    return 0;
}
...
int assert_file_overwrite(const char *filename)
{
    ...
    if (file_overwrite && no_file_overwrite) {
        fprintf(__stderrp, "Error, both -y and -n supplied. Exiting.\n");
        return (-(22));
    }
    ...
    if (!file_overwrite) {
        if (proto_name && !strcmp(proto_name, "file") && avio_check(filename, 0) == 0) {
            if (stdin_interaction && !no_file_overwrite) {
                fprintf(__stderrp,"File '%s' already exists. Overwrite? [y/N] ", filename);
                fflush(__stderrp);
                term_exit();
                signal(2, (void (*)(int))0);
                if (!read_yesno()) {
                    av_log(((void*)0), 8, "Not overwriting - exiting\n");
                    return (-(int)(('E') | (('X') << 8) | (('I') << 16) | ((unsigned)('T') << 24)));
                }
                term_init();
            }
            else {
                av_log(((void*)0), 8, "File '%s' already exists. Exiting.\n", filename);
                return (-(int)(('E') | (('X') << 8) | (('I') << 16) | ((unsigned)('T') << 24)));
            }
        }
    }
    ...
    if (proto_name && !strcmp(proto_name, "file")) {
        for (int i = 0; i < nb_input_files; i++) {
             ...
             if (!strcmp(filename, file->ctx->url)) {
                 av_log(((void*)0), 8, "Output %s same as Input #%d - exiting\n", filename, i);
                 av_log(((void*)0), 24, "FFmpeg cannot edit existing files in-place.\n");
                 return (-(22));
             }
        }
    }
...
static int opt_target(void *optctx, const char *opt, const char *arg)
{
    ...
    if (!strncmp(arg, "pal-", 4)) {
        ...
        if (norm != UNKNOWN)
            av_log(((void*)0), 32, "Assuming %s for target.\n", norm == PAL ? "PAL" : "NTSC");
    }

    if (norm == UNKNOWN) {
        av_log(((void*)0), 8, "Could not determine norm (PAL/NTSC/NTSC-Film) for target.\n");
        av_log(((void*)0), 8, "Please prefix target with \"pal-\", \"ntsc-\" or \"film-\",\n");
        av_log(((void*)0), 8, "or set a framerate with \"-r xxx\".\n");
        return (-(22));
    }
    ...
    if (!strcmp(arg, "vcd")) {
        ...
        av_log(((void*)0), 16, "Unknown target: %s\n", arg);
        return (-(22));
    }
...
static int opt_vstats(void *optctx, const char *opt, const char *arg)
{
    ...
    if (!today) {
        av_log(((void*)0), 8, "Unable to get current time: %s\n", strerror((*__error())));
        return (-((*__error())));
    }

    __builtin___snprintf_chk (filename, sizeof(filename), 0, __builtin_object_size (filename, 2 > 1 ? 1 : 0), "vstats_%02d%02d%02d.log", today->tm_hour, today->tm_min, today->tm_sec);
...
static int opt_preset(void *optctx, const char *opt, const char *arg)
{
    ...
    if (!(f = get_preset_file(filename, sizeof(filename), arg, *opt == 'f', codec_name))) {
        if(!strncmp(arg, "libx264-lossless", strlen("libx264-lossless"))){
            av_log(((void*)0), 8, "Please use -preset <speed> -qp 0\n");
        }else
            av_log(((void*)0), 8, "File for preset '%s' not found\n", arg);
        return (-(2));
    }
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (!av_strtok(key, "=", &value) ||
            !av_strtok(value, "\r\n", &endptr)) {
            av_log(((void*)0), 8, "%s: Invalid syntax: '%s'\n", filename, line);
            ret = (-(22));
            goto fail;
        }
        av_log(((void*)0), 48, "ffpreset[%s]: set '%s' = '%s'\n", filename, key, value);
        ...
        if (!strcmp(key, "acodec")) opt_audio_codec (o, key, value);
            ...
            av_log(((void*)0), 8, "%s: Invalid option or argument: '%s', parsed as '%s' = '%s'\n",
...
static int opt_bitrate(void *optctx, const char *opt, const char *arg)
{
    ...
    if(!strcmp(opt, "ab")){
        ...
        av_log(((void*)0), 24, "Please use -b:a or -b:v, -b is ambiguous\n");
...
static int opt_qscale(void *optctx, const char *opt, const char *arg)
{
    ...
    if(!strcmp(opt, "qscale")){
        av_log(((void*)0), 24, "Please use -q:a or -q:v, -qscale is ambiguous\n");
...
static int opt_profile(void *optctx, const char *opt, const char *arg)
{
    ...
    if(!strcmp(opt, "profile")){
        av_log(((void*)0), 24, "Please use -profile:a or -profile:v, -profile is ambiguous\n");
...
void show_help_default(const char *opt, const char *arg)
{
    ...
    if (opt && *opt) {
        if (!strcmp(opt, "long"))
            ...
            av_log(((void*)0), 16, "Unknown help option '%s'.\n", opt);
    }
    ...
    if (show_avoptions) {
        ...
        show_help_children(swr_get_class(), 8);

        show_help_children(avfilter_get_class(), 16 | 8 | (1<<16));
        show_help_children(av_bsf_get_class(), 16 | 8 | (1<<8));
    }
}
                                       ...
                                       Scheduler*))
{
    ...
    for (i = 0; i < l->nb_groups; i++) {
        ...
        av_log(((void*)0), 48, "Opening an %s file: %s.\n", inout, g->arg);
        ret = open_file(&o, g->arg, sch);
        uninit_options(&o);
        if (ret < 0) {
            av_log(((void*)0), 16, "Error opening %s file %s.\n",
                   inout, g->arg);
            return ret;
        }
        av_log(((void*)0), 48, "Successfully opened the file.\n");
    }
...
int ffmpeg_parse_options(int argc, char **argv, Scheduler *sch)
{
    ...
    __builtin___memset_chk (&octx, 0, sizeof(octx), __builtin_object_size (&octx, 0));
...
static int opt_progress(void *optctx, const char *opt, const char *arg)
{
    ...
    if (!strcmp(arg, "-"))
        arg = "pipe:";
    ret = avio_open2(&avio, arg, 2, &int_cb, ((void*)0));
    if (ret < 0) {
        av_log(((void*)0), 16, "Failed to open progress URL \"%s\": %s\n",
...
{
    av_log(((void*)0), 24, "Option -%s is deprecated and has no effect\n", opt);
    return 0;
}
