# 1 "/Users/davidchen/repo/ffmpeg/libavfilter/af_aiir.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 418 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/Users/davidchen/repo/ffmpeg/libavfilter/af_aiir.c" 2
# 21 "/Users/davidchen/repo/ffmpeg/libavfilter/af_aiir.c"
...
# 22 "/Users/davidchen/repo/ffmpeg/libavfilter/af_aiir.c" 2
...
# 24 "/Users/davidchen/repo/ffmpeg/libavfilter/af_aiir.c" 2
...
# 25 "/Users/davidchen/repo/ffmpeg/libavfilter/af_aiir.c" 2
# 1 "./libavutil/opt.h" 1
# 30 "./libavutil/opt.h"
...
int av_opt_serialize(void *obj, int opt_flags, int flags, char **buffer,
                     const char key_val_sep, const char pairs_sep);
# 26 "/Users/davidchen/repo/ffmpeg/libavfilter/af_aiir.c" 2
...
# 27 "/Users/davidchen/repo/ffmpeg/libavfilter/af_aiir.c" 2
...
# 28 "/Users/davidchen/repo/ffmpeg/libavfilter/af_aiir.c" 2
...
# 30 "/Users/davidchen/repo/ffmpeg/libavfilter/af_aiir.c" 2
...
# 32 "/Users/davidchen/repo/ffmpeg/libavfilter/af_aiir.c" 2
...
# 165 "/Users/davidchen/repo/ffmpeg/libavfilter/af_aiir.c"
static int iir_ch_s16p(AVFilterContext *ctx, void *arg, int ch, int nb_jobs) { AudioIIRContext *s = ctx->priv; const double ig = s->dry_gain; const double og = s->wet_gain; const double mix = s->mix; ThreadData *td = arg; AVFrame *in = td->in, *out = td->out; const int16_t *src = (const int16_t *)in->extended_data[ch]; double *oc = (double *)s->iir[ch].cache[0]; double *ic = (double *)s->iir[ch].cache[1]; const int nb_a = s->iir[ch].nb_ab[0]; const int nb_b = s->iir[ch].nb_ab[1]; const double *a = s->iir[ch].ab[0]; const double *b = s->iir[ch].ab[1]; const double g = s->iir[ch].g; int *clippings = &s->iir[ch].clippings; int16_t *dst = (int16_t *)out->extended_data[ch]; int n; for (n = 0; n < in->nb_samples; n++) { double sample = 0.; int x; __builtin___memmove_chk (&ic[1], &ic[0], (nb_b - 1) * sizeof(*ic), __builtin_object_size (&ic[1], 0)); __builtin___memmove_chk (&oc[1], &oc[0], (nb_a - 1) * sizeof(*oc), __builtin_object_size (&oc[1], 0)); ic[0] = src[n] * ig; for (x = 0; x < nb_b; x++) sample += b[x] * ic[x]; for (x = 1; x < nb_a; x++) sample -= a[x] * oc[x]; oc[0] = sample; sample *= og * g; sample = sample * mix + ic[0] * (1. - mix); if (1 && sample < -32768) { (*clippings)++; dst[n] = -32768; } else if (1 && sample > 32767) { (*clippings)++; dst[n] = 32767; } else { dst[n] = sample; } } return 0; }
static int iir_ch_s32p(AVFilterContext *ctx, void *arg, int ch, int nb_jobs) { AudioIIRContext *s = ctx->priv; const double ig = s->dry_gain; const double og = s->wet_gain; const double mix = s->mix; ThreadData *td = arg; AVFrame *in = td->in, *out = td->out; const int32_t *src = (const int32_t *)in->extended_data[ch]; double *oc = (double *)s->iir[ch].cache[0]; double *ic = (double *)s->iir[ch].cache[1]; const int nb_a = s->iir[ch].nb_ab[0]; const int nb_b = s->iir[ch].nb_ab[1]; const double *a = s->iir[ch].ab[0]; const double *b = s->iir[ch].ab[1]; const double g = s->iir[ch].g; int *clippings = &s->iir[ch].clippings; int32_t *dst = (int32_t *)out->extended_data[ch]; int n; for (n = 0; n < in->nb_samples; n++) { double sample = 0.; int x; __builtin___memmove_chk (&ic[1], &ic[0], (nb_b - 1) * sizeof(*ic), __builtin_object_size (&ic[1], 0)); __builtin___memmove_chk (&oc[1], &oc[0], (nb_a - 1) * sizeof(*oc), __builtin_object_size (&oc[1], 0)); ic[0] = src[n] * ig; for (x = 0; x < nb_b; x++) sample += b[x] * ic[x]; for (x = 1; x < nb_a; x++) sample -= a[x] * oc[x]; oc[0] = sample; sample *= og * g; sample = sample * mix + ic[0] * (1. - mix); if (1 && sample < (-2147483647 -1)) { (*clippings)++; dst[n] = (-2147483647 -1); } else if (1 && sample > 2147483647) { (*clippings)++; dst[n] = 2147483647; } else { dst[n] = sample; } } return 0; }
static int iir_ch_fltp(AVFilterContext *ctx, void *arg, int ch, int nb_jobs) { AudioIIRContext *s = ctx->priv; const double ig = s->dry_gain; const double og = s->wet_gain; const double mix = s->mix; ThreadData *td = arg; AVFrame *in = td->in, *out = td->out; const float *src = (const float *)in->extended_data[ch]; double *oc = (double *)s->iir[ch].cache[0]; double *ic = (double *)s->iir[ch].cache[1]; const int nb_a = s->iir[ch].nb_ab[0]; const int nb_b = s->iir[ch].nb_ab[1]; const double *a = s->iir[ch].ab[0]; const double *b = s->iir[ch].ab[1]; const double g = s->iir[ch].g; int *clippings = &s->iir[ch].clippings; float *dst = (float *)out->extended_data[ch]; int n; for (n = 0; n < in->nb_samples; n++) { double sample = 0.; int x; __builtin___memmove_chk (&ic[1], &ic[0], (nb_b - 1) * sizeof(*ic), __builtin_object_size (&ic[1], 0)); __builtin___memmove_chk (&oc[1], &oc[0], (nb_a - 1) * sizeof(*oc), __builtin_object_size (&oc[1], 0)); ic[0] = src[n] * ig; for (x = 0; x < nb_b; x++) sample += b[x] * ic[x]; for (x = 1; x < nb_a; x++) sample -= a[x] * oc[x]; oc[0] = sample; sample *= og * g; sample = sample * mix + ic[0] * (1. - mix); if (0 && sample < -1.) { (*clippings)++; dst[n] = -1.; } else if (0 && sample > 1.) { (*clippings)++; dst[n] = 1.; } else { dst[n] = sample; } } return 0; }
static int iir_ch_dblp(AVFilterContext *ctx, void *arg, int ch, int nb_jobs) { AudioIIRContext *s = ctx->priv; const double ig = s->dry_gain; const double og = s->wet_gain; const double mix = s->mix; ThreadData *td = arg; AVFrame *in = td->in, *out = td->out; const double *src = (const double *)in->extended_data[ch]; double *oc = (double *)s->iir[ch].cache[0]; double *ic = (double *)s->iir[ch].cache[1]; const int nb_a = s->iir[ch].nb_ab[0]; const int nb_b = s->iir[ch].nb_ab[1]; const double *a = s->iir[ch].ab[0]; const double *b = s->iir[ch].ab[1]; const double g = s->iir[ch].g; int *clippings = &s->iir[ch].clippings; double *dst = (double *)out->extended_data[ch]; int n; for (n = 0; n < in->nb_samples; n++) { double sample = 0.; int x; __builtin___memmove_chk (&ic[1], &ic[0], (nb_b - 1) * sizeof(*ic), __builtin_object_size (&ic[1], 0)); __builtin___memmove_chk (&oc[1], &oc[0], (nb_a - 1) * sizeof(*oc), __builtin_object_size (&oc[1], 0)); ic[0] = src[n] * ig; for (x = 0; x < nb_b; x++) sample += b[x] * ic[x]; for (x = 1; x < nb_a; x++) sample -= a[x] * oc[x]; oc[0] = sample; sample *= og * g; sample = sample * mix + ic[0] * (1. - mix); if (0 && sample < -1.) { (*clippings)++; dst[n] = -1.; } else if (0 && sample > 1.) { (*clippings)++; dst[n] = 1.; } else { dst[n] = sample; } } return 0; }
# 224 "/Users/davidchen/repo/ffmpeg/libavfilter/af_aiir.c"
static int iir_ch_serial_s16p(AVFilterContext *ctx, void *arg, int ch, int nb_jobs) { AudioIIRContext *s = ctx->priv; const double ig = s->dry_gain; const double og = s->wet_gain; const double mix = s->mix; const double imix = 1. - mix; ThreadData *td = arg; AVFrame *in = td->in, *out = td->out; const int16_t *src = (const int16_t *)in->extended_data[ch]; int16_t *dst = (int16_t *)out->extended_data[ch]; IIRChannel *iir = &s->iir[ch]; const double g = iir->g; int *clippings = &iir->clippings; int nb_biquads = (((iir->nb_ab[0]) > (iir->nb_ab[1]) ? (iir->nb_ab[0]) : (iir->nb_ab[1])) + 1) / 2; int n, i; for (i = nb_biquads - 1; i >= 0; i--) { const double a1 = -iir->biquads[i].a[1]; const double a2 = -iir->biquads[i].a[2]; const double b0 = iir->biquads[i].b[0]; const double b1 = iir->biquads[i].b[1]; const double b2 = iir->biquads[i].b[2]; double w1 = iir->biquads[i].w1; double w2 = iir->biquads[i].w2; for (n = 0; n < in->nb_samples; n++) { double i0 = ig * (i ? dst[n] : src[n]); double o0 = i0 * b0 + w1; w1 = b1 * i0 + w2 + a1 * o0; w2 = b2 * i0 + a2 * o0; o0 *= og * g; o0 = o0 * mix + imix * i0; if (1 && o0 < -32768) { (*clippings)++; dst[n] = -32768; } else if (1 && o0 > 32767) { (*clippings)++; dst[n] = 32767; } else { dst[n] = o0; } } iir->biquads[i].w1 = w1; iir->biquads[i].w2 = w2; } return 0; }
static int iir_ch_serial_s32p(AVFilterContext *ctx, void *arg, int ch, int nb_jobs) { AudioIIRContext *s = ctx->priv; const double ig = s->dry_gain; const double og = s->wet_gain; const double mix = s->mix; const double imix = 1. - mix; ThreadData *td = arg; AVFrame *in = td->in, *out = td->out; const int32_t *src = (const int32_t *)in->extended_data[ch]; int32_t *dst = (int32_t *)out->extended_data[ch]; IIRChannel *iir = &s->iir[ch]; const double g = iir->g; int *clippings = &iir->clippings; int nb_biquads = (((iir->nb_ab[0]) > (iir->nb_ab[1]) ? (iir->nb_ab[0]) : (iir->nb_ab[1])) + 1) / 2; int n, i; for (i = nb_biquads - 1; i >= 0; i--) { const double a1 = -iir->biquads[i].a[1]; const double a2 = -iir->biquads[i].a[2]; const double b0 = iir->biquads[i].b[0]; const double b1 = iir->biquads[i].b[1]; const double b2 = iir->biquads[i].b[2]; double w1 = iir->biquads[i].w1; double w2 = iir->biquads[i].w2; for (n = 0; n < in->nb_samples; n++) { double i0 = ig * (i ? dst[n] : src[n]); double o0 = i0 * b0 + w1; w1 = b1 * i0 + w2 + a1 * o0; w2 = b2 * i0 + a2 * o0; o0 *= og * g; o0 = o0 * mix + imix * i0; if (1 && o0 < (-2147483647 -1)) { (*clippings)++; dst[n] = (-2147483647 -1); } else if (1 && o0 > 2147483647) { (*clippings)++; dst[n] = 2147483647; } else { dst[n] = o0; } } iir->biquads[i].w1 = w1; iir->biquads[i].w2 = w2; } return 0; }
static int iir_ch_serial_fltp(AVFilterContext *ctx, void *arg, int ch, int nb_jobs) { AudioIIRContext *s = ctx->priv; const double ig = s->dry_gain; const double og = s->wet_gain; const double mix = s->mix; const double imix = 1. - mix; ThreadData *td = arg; AVFrame *in = td->in, *out = td->out; const float *src = (const float *)in->extended_data[ch]; float *dst = (float *)out->extended_data[ch]; IIRChannel *iir = &s->iir[ch]; const double g = iir->g; int *clippings = &iir->clippings; int nb_biquads = (((iir->nb_ab[0]) > (iir->nb_ab[1]) ? (iir->nb_ab[0]) : (iir->nb_ab[1])) + 1) / 2; int n, i; for (i = nb_biquads - 1; i >= 0; i--) { const double a1 = -iir->biquads[i].a[1]; const double a2 = -iir->biquads[i].a[2]; const double b0 = iir->biquads[i].b[0]; const double b1 = iir->biquads[i].b[1]; const double b2 = iir->biquads[i].b[2]; double w1 = iir->biquads[i].w1; double w2 = iir->biquads[i].w2; for (n = 0; n < in->nb_samples; n++) { double i0 = ig * (i ? dst[n] : src[n]); double o0 = i0 * b0 + w1; w1 = b1 * i0 + w2 + a1 * o0; w2 = b2 * i0 + a2 * o0; o0 *= og * g; o0 = o0 * mix + imix * i0; if (0 && o0 < -1.) { (*clippings)++; dst[n] = -1.; } else if (0 && o0 > 1.) { (*clippings)++; dst[n] = 1.; } else { dst[n] = o0; } } iir->biquads[i].w1 = w1; iir->biquads[i].w2 = w2; } return 0; }
static int iir_ch_serial_dblp(AVFilterContext *ctx, void *arg, int ch, int nb_jobs) { AudioIIRContext *s = ctx->priv; const double ig = s->dry_gain; const double og = s->wet_gain; const double mix = s->mix; const double imix = 1. - mix; ThreadData *td = arg; AVFrame *in = td->in, *out = td->out; const double *src = (const double *)in->extended_data[ch]; double *dst = (double *)out->extended_data[ch]; IIRChannel *iir = &s->iir[ch]; const double g = iir->g; int *clippings = &iir->clippings; int nb_biquads = (((iir->nb_ab[0]) > (iir->nb_ab[1]) ? (iir->nb_ab[0]) : (iir->nb_ab[1])) + 1) / 2; int n, i; for (i = nb_biquads - 1; i >= 0; i--) { const double a1 = -iir->biquads[i].a[1]; const double a2 = -iir->biquads[i].a[2]; const double b0 = iir->biquads[i].b[0]; const double b1 = iir->biquads[i].b[1]; const double b2 = iir->biquads[i].b[2]; double w1 = iir->biquads[i].w1; double w2 = iir->biquads[i].w2; for (n = 0; n < in->nb_samples; n++) { double i0 = ig * (i ? dst[n] : src[n]); double o0 = i0 * b0 + w1; w1 = b1 * i0 + w2 + a1 * o0; w2 = b2 * i0 + a2 * o0; o0 *= og * g; o0 = o0 * mix + imix * i0; if (0 && o0 < -1.) { (*clippings)++; dst[n] = -1.; } else if (0 && o0 > 1.) { (*clippings)++; dst[n] = 1.; } else { dst[n] = o0; } } iir->biquads[i].w1 = w1; iir->biquads[i].w2 = w2; } return 0; }
# 288 "/Users/davidchen/repo/ffmpeg/libavfilter/af_aiir.c"
static int iir_ch_parallel_s16p(AVFilterContext *ctx, void *arg, int ch, int nb_jobs) { AudioIIRContext *s = ctx->priv; const double ig = s->dry_gain; const double og = s->wet_gain; const double mix = s->mix; const double imix = 1. - mix; ThreadData *td = arg; AVFrame *in = td->in, *out = td->out; const int16_t *src = (const int16_t *)in->extended_data[ch]; int16_t *dst = (int16_t *)out->extended_data[ch]; IIRChannel *iir = &s->iir[ch]; const double g = iir->g; const double fir = iir->fir; int *clippings = &iir->clippings; int nb_biquads = (((iir->nb_ab[0]) > (iir->nb_ab[1]) ? (iir->nb_ab[0]) : (iir->nb_ab[1])) + 1) / 2; int n, i; for (i = 0; i < nb_biquads; i++) { const double a1 = -iir->biquads[i].a[1]; const double a2 = -iir->biquads[i].a[2]; const double b1 = iir->biquads[i].b[1]; const double b2 = iir->biquads[i].b[2]; double w1 = iir->biquads[i].w1; double w2 = iir->biquads[i].w2; for (n = 0; n < in->nb_samples; n++) { double i0 = ig * src[n]; double o0 = w1; w1 = b1 * i0 + w2 + a1 * o0; w2 = b2 * i0 + a2 * o0; o0 *= og * g; o0 += dst[n]; if (1 && o0 < -32768) { (*clippings)++; dst[n] = -32768; } else if (1 && o0 > 32767) { (*clippings)++; dst[n] = 32767; } else { dst[n] = o0; } } iir->biquads[i].w1 = w1; iir->biquads[i].w2 = w2; } for (n = 0; n < in->nb_samples; n++) { dst[n] += fir * src[n]; dst[n] = dst[n] * mix + imix * src[n]; } return 0; }
static int iir_ch_parallel_s32p(AVFilterContext *ctx, void *arg, int ch, int nb_jobs) { AudioIIRContext *s = ctx->priv; const double ig = s->dry_gain; const double og = s->wet_gain; const double mix = s->mix; const double imix = 1. - mix; ThreadData *td = arg; AVFrame *in = td->in, *out = td->out; const int32_t *src = (const int32_t *)in->extended_data[ch]; int32_t *dst = (int32_t *)out->extended_data[ch]; IIRChannel *iir = &s->iir[ch]; const double g = iir->g; const double fir = iir->fir; int *clippings = &iir->clippings; int nb_biquads = (((iir->nb_ab[0]) > (iir->nb_ab[1]) ? (iir->nb_ab[0]) : (iir->nb_ab[1])) + 1) / 2; int n, i; for (i = 0; i < nb_biquads; i++) { const double a1 = -iir->biquads[i].a[1]; const double a2 = -iir->biquads[i].a[2]; const double b1 = iir->biquads[i].b[1]; const double b2 = iir->biquads[i].b[2]; double w1 = iir->biquads[i].w1; double w2 = iir->biquads[i].w2; for (n = 0; n < in->nb_samples; n++) { double i0 = ig * src[n]; double o0 = w1; w1 = b1 * i0 + w2 + a1 * o0; w2 = b2 * i0 + a2 * o0; o0 *= og * g; o0 += dst[n]; if (1 && o0 < (-2147483647 -1)) { (*clippings)++; dst[n] = (-2147483647 -1); } else if (1 && o0 > 2147483647) { (*clippings)++; dst[n] = 2147483647; } else { dst[n] = o0; } } iir->biquads[i].w1 = w1; iir->biquads[i].w2 = w2; } for (n = 0; n < in->nb_samples; n++) { dst[n] += fir * src[n]; dst[n] = dst[n] * mix + imix * src[n]; } return 0; }
static int iir_ch_parallel_fltp(AVFilterContext *ctx, void *arg, int ch, int nb_jobs) { AudioIIRContext *s = ctx->priv; const double ig = s->dry_gain; const double og = s->wet_gain; const double mix = s->mix; const double imix = 1. - mix; ThreadData *td = arg; AVFrame *in = td->in, *out = td->out; const float *src = (const float *)in->extended_data[ch]; float *dst = (float *)out->extended_data[ch]; IIRChannel *iir = &s->iir[ch]; const double g = iir->g; const double fir = iir->fir; int *clippings = &iir->clippings; int nb_biquads = (((iir->nb_ab[0]) > (iir->nb_ab[1]) ? (iir->nb_ab[0]) : (iir->nb_ab[1])) + 1) / 2; int n, i; for (i = 0; i < nb_biquads; i++) { const double a1 = -iir->biquads[i].a[1]; const double a2 = -iir->biquads[i].a[2]; const double b1 = iir->biquads[i].b[1]; const double b2 = iir->biquads[i].b[2]; double w1 = iir->biquads[i].w1; double w2 = iir->biquads[i].w2; for (n = 0; n < in->nb_samples; n++) { double i0 = ig * src[n]; double o0 = w1; w1 = b1 * i0 + w2 + a1 * o0; w2 = b2 * i0 + a2 * o0; o0 *= og * g; o0 += dst[n]; if (0 && o0 < -1.) { (*clippings)++; dst[n] = -1.; } else if (0 && o0 > 1.) { (*clippings)++; dst[n] = 1.; } else { dst[n] = o0; } } iir->biquads[i].w1 = w1; iir->biquads[i].w2 = w2; } for (n = 0; n < in->nb_samples; n++) { dst[n] += fir * src[n]; dst[n] = dst[n] * mix + imix * src[n]; } return 0; }
static int iir_ch_parallel_dblp(AVFilterContext *ctx, void *arg, int ch, int nb_jobs) { AudioIIRContext *s = ctx->priv; const double ig = s->dry_gain; const double og = s->wet_gain; const double mix = s->mix; const double imix = 1. - mix; ThreadData *td = arg; AVFrame *in = td->in, *out = td->out; const double *src = (const double *)in->extended_data[ch]; double *dst = (double *)out->extended_data[ch]; IIRChannel *iir = &s->iir[ch]; const double g = iir->g; const double fir = iir->fir; int *clippings = &iir->clippings; int nb_biquads = (((iir->nb_ab[0]) > (iir->nb_ab[1]) ? (iir->nb_ab[0]) : (iir->nb_ab[1])) + 1) / 2; int n, i; for (i = 0; i < nb_biquads; i++) { const double a1 = -iir->biquads[i].a[1]; const double a2 = -iir->biquads[i].a[2]; const double b1 = iir->biquads[i].b[1]; const double b2 = iir->biquads[i].b[2]; double w1 = iir->biquads[i].w1; double w2 = iir->biquads[i].w2; for (n = 0; n < in->nb_samples; n++) { double i0 = ig * src[n]; double o0 = w1; w1 = b1 * i0 + w2 + a1 * o0; w2 = b2 * i0 + a2 * o0; o0 *= og * g; o0 += dst[n]; if (0 && o0 < -1.) { (*clippings)++; dst[n] = -1.; } else if (0 && o0 > 1.) { (*clippings)++; dst[n] = 1.; } else { dst[n] = o0; } } iir->biquads[i].w1 = w1; iir->biquads[i].w2 = w2; } for (n = 0; n < in->nb_samples; n++) { dst[n] += fir * src[n]; dst[n] = dst[n] * mix + imix * src[n]; } return 0; }
# 345 "/Users/davidchen/repo/ffmpeg/libavfilter/af_aiir.c"
static int iir_ch_lattice_s16p(AVFilterContext *ctx, void *arg, int ch, int nb_jobs) { AudioIIRContext *s = ctx->priv; const double ig = s->dry_gain; const double og = s->wet_gain; const double mix = s->mix; ThreadData *td = arg; AVFrame *in = td->in, *out = td->out; const int16_t *src = (const int16_t *)in->extended_data[ch]; double n0, n1, p0, *x = (double *)s->iir[ch].cache[0]; const int nb_stages = s->iir[ch].nb_ab[1]; const double *v = s->iir[ch].ab[0]; const double *k = s->iir[ch].ab[1]; const double g = s->iir[ch].g; int *clippings = &s->iir[ch].clippings; int16_t *dst = (int16_t *)out->extended_data[ch]; int n; for (n = 0; n < in->nb_samples; n++) { const double in = src[n] * ig; double out = 0.; n1 = in; for (int i = nb_stages - 1; i >= 0; i--) { n0 = n1 - k[i] * x[i]; p0 = n0 * k[i] + x[i]; out += p0 * v[i+1]; x[i] = p0; n1 = n0; } out += n1 * v[0]; __builtin___memmove_chk (&x[1], &x[0], nb_stages * sizeof(*x), __builtin_object_size (&x[1], 0)); x[0] = n1; out *= og * g; out = out * mix + in * (1. - mix); if (1 && out < -32768) { (*clippings)++; dst[n] = -32768; } else if (1 && out > 32767) { (*clippings)++; dst[n] = 32767; } else { dst[n] = out; } } return 0; }
static int iir_ch_lattice_s32p(AVFilterContext *ctx, void *arg, int ch, int nb_jobs) { AudioIIRContext *s = ctx->priv; const double ig = s->dry_gain; const double og = s->wet_gain; const double mix = s->mix; ThreadData *td = arg; AVFrame *in = td->in, *out = td->out; const int32_t *src = (const int32_t *)in->extended_data[ch]; double n0, n1, p0, *x = (double *)s->iir[ch].cache[0]; const int nb_stages = s->iir[ch].nb_ab[1]; const double *v = s->iir[ch].ab[0]; const double *k = s->iir[ch].ab[1]; const double g = s->iir[ch].g; int *clippings = &s->iir[ch].clippings; int32_t *dst = (int32_t *)out->extended_data[ch]; int n; for (n = 0; n < in->nb_samples; n++) { const double in = src[n] * ig; double out = 0.; n1 = in; for (int i = nb_stages - 1; i >= 0; i--) { n0 = n1 - k[i] * x[i]; p0 = n0 * k[i] + x[i]; out += p0 * v[i+1]; x[i] = p0; n1 = n0; } out += n1 * v[0]; __builtin___memmove_chk (&x[1], &x[0], nb_stages * sizeof(*x), __builtin_object_size (&x[1], 0)); x[0] = n1; out *= og * g; out = out * mix + in * (1. - mix); if (1 && out < (-2147483647 -1)) { (*clippings)++; dst[n] = (-2147483647 -1); } else if (1 && out > 2147483647) { (*clippings)++; dst[n] = 2147483647; } else { dst[n] = out; } } return 0; }
static int iir_ch_lattice_fltp(AVFilterContext *ctx, void *arg, int ch, int nb_jobs) { AudioIIRContext *s = ctx->priv; const double ig = s->dry_gain; const double og = s->wet_gain; const double mix = s->mix; ThreadData *td = arg; AVFrame *in = td->in, *out = td->out; const float *src = (const float *)in->extended_data[ch]; double n0, n1, p0, *x = (double *)s->iir[ch].cache[0]; const int nb_stages = s->iir[ch].nb_ab[1]; const double *v = s->iir[ch].ab[0]; const double *k = s->iir[ch].ab[1]; const double g = s->iir[ch].g; int *clippings = &s->iir[ch].clippings; float *dst = (float *)out->extended_data[ch]; int n; for (n = 0; n < in->nb_samples; n++) { const double in = src[n] * ig; double out = 0.; n1 = in; for (int i = nb_stages - 1; i >= 0; i--) { n0 = n1 - k[i] * x[i]; p0 = n0 * k[i] + x[i]; out += p0 * v[i+1]; x[i] = p0; n1 = n0; } out += n1 * v[0]; __builtin___memmove_chk (&x[1], &x[0], nb_stages * sizeof(*x), __builtin_object_size (&x[1], 0)); x[0] = n1; out *= og * g; out = out * mix + in * (1. - mix); if (0 && out < -1.) { (*clippings)++; dst[n] = -1.; } else if (0 && out > 1.) { (*clippings)++; dst[n] = 1.; } else { dst[n] = out; } } return 0; }
static int iir_ch_lattice_dblp(AVFilterContext *ctx, void *arg, int ch, int nb_jobs) { AudioIIRContext *s = ctx->priv; const double ig = s->dry_gain; const double og = s->wet_gain; const double mix = s->mix; ThreadData *td = arg; AVFrame *in = td->in, *out = td->out; const double *src = (const double *)in->extended_data[ch]; double n0, n1, p0, *x = (double *)s->iir[ch].cache[0]; const int nb_stages = s->iir[ch].nb_ab[1]; const double *v = s->iir[ch].ab[0]; const double *k = s->iir[ch].ab[1]; const double g = s->iir[ch].g; int *clippings = &s->iir[ch].clippings; double *dst = (double *)out->extended_data[ch]; int n; for (n = 0; n < in->nb_samples; n++) { const double in = src[n] * ig; double out = 0.; n1 = in; for (int i = nb_stages - 1; i >= 0; i--) { n0 = n1 - k[i] * x[i]; p0 = n0 * k[i] + x[i]; out += p0 * v[i+1]; x[i] = p0; n1 = n0; } out += n1 * v[0]; __builtin___memmove_chk (&x[1], &x[0], nb_stages * sizeof(*x), __builtin_object_size (&x[1], 0)); x[0] = n1; out *= og * g; out = out * mix + in * (1. - mix); if (0 && out < -1.) { (*clippings)++; dst[n] = -1.; } else if (0 && out > 1.) { (*clippings)++; dst[n] = 1.; } else { dst[n] = out; } } return 0; }
...
static int read_gains(AVFilterContext *ctx, char *item_str, int nb_items)
{
    ...
    for (i = 0; i < nb_items; i++) {
        ...
        if (av_sscanf(arg, "%lf", &s->iir[i].g) != 1) {
            av_log(ctx, 16, "Invalid gains supplied: %s\n", arg);
            av_freep(&old_str);
            return (-(22));
        }
...
static int read_tf_coefficients(AVFilterContext *ctx, char *item_str, int nb_items, double *dst)
{
    ...
    for (i = 0; i < nb_items; i++) {
        ...
        if (av_sscanf(arg, "%lf", &dst[i]) != 1) {
            av_log(ctx, 16, "Invalid coefficients supplied: %s\n", arg);
            av_freep(&old_str);
            return (-(22));
        }
    }
...
static int expand(AVFilterContext *ctx, double *pz, int n, double *coefs)
{
    ...
    for (int i = 0; i < n + 1; i++) {
        if (fabs(coefs[2 * i + 1]) > 1.19209290e-7F) {
            av_log(ctx, 16, "coefs: %f of z^%d is not real; poles/zeros are not complex conjugates.\n",
                   coefs[2 * i + 1], i);
            return (-(22));
        }
    }
...
static int decompose_zp2biquads(AVFilterContext *ctx, int channels)
{
    ...
    for (ch = 0; ch < channels; ch++) {
        IIRChannel *iir = &s->iir[ch];
        int nb_biquads = (((iir->nb_ab[0]) > (iir->nb_ab[1]) ? (iir->nb_ab[0]) : (iir->nb_ab[1])) + 1) / 2;
        ...
        while (nb_biquads--) {
            ...
            for (i = 0; i < iir->nb_ab[0]; i++) {
                ...
                if (( sizeof(iir->ab[0][2 * i]) == sizeof(float) ? __inline_isnanf((float)(iir->ab[0][2 * i])) : sizeof(iir->ab[0][2 * i]) == sizeof(double) ? __inline_isnand((double)(iir->ab[0][2 * i])) : __inline_isnanl((long double)(iir->ab[0][2 * i]))) || ( sizeof(iir->ab[0][2 * i + 1]) == sizeof(float) ? __inline_isnanf((float)(iir->ab[0][2 * i + 1])) : sizeof(iir->ab[0][2 * i + 1]) == sizeof(double) ? __inline_isnand((double)(iir->ab[0][2 * i + 1])) : __inline_isnanl((long double)(iir->ab[0][2 * i + 1]))))
                    continue;
            ...
            av_log(ctx, 40, "outmost_pole is %d.%d\n", outmost_pole.a, outmost_pole.b);
            ...
            for (i = 0; i < iir->nb_ab[1]; i++) {
                ...
                if (( sizeof(iir->ab[1][2 * i]) == sizeof(float) ? __inline_isnanf((float)(iir->ab[1][2 * i])) : sizeof(iir->ab[1][2 * i]) == sizeof(double) ? __inline_isnand((double)(iir->ab[1][2 * i])) : __inline_isnanl((long double)(iir->ab[1][2 * i]))) || ( sizeof(iir->ab[1][2 * i + 1]) == sizeof(float) ? __inline_isnanf((float)(iir->ab[1][2 * i + 1])) : sizeof(iir->ab[1][2 * i + 1]) == sizeof(double) ? __inline_isnand((double)(iir->ab[1][2 * i + 1])) : __inline_isnanl((long double)(iir->ab[1][2 * i + 1]))))
                    continue;
            ...
            av_log(ctx, 40, "nearest_zero is %d.%d\n", nearest_zero.a, nearest_zero.b);
            ...
            iir->ab[0][2 * outmost_pole.a] = iir->ab[0][2 * outmost_pole.a + 1] = __builtin_nanf("0x7fc00000");
            iir->ab[0][2 * outmost_pole.b] = iir->ab[0][2 * outmost_pole.b + 1] = __builtin_nanf("0x7fc00000");
            iir->ab[1][2 * nearest_zero.a] = iir->ab[1][2 * nearest_zero.a + 1] = __builtin_nanf("0x7fc00000");
            iir->ab[1][2 * nearest_zero.b] = iir->ab[1][2 * nearest_zero.b + 1] = __builtin_nanf("0x7fc00000");
...
static int convert_serial2parallel(AVFilterContext *ctx, int channels)
{
    ...
    for (int ch = 0; ch < channels; ch++) {
        ...
        for (int n = 0; n < nb_biquads; n++) {
            ...
            __builtin___memcpy_chk (M + n * 2 * (length - 1), resp, sizeof(*resp) * (length - 1), __builtin_object_size (M + n * 2 * (length - 1), 0));
            __builtin___memcpy_chk (M + n * 2 * (length - 1) + length, resp, sizeof(*resp) * (length - 2), __builtin_object_size (M + n * 2 * (length - 1) + length, 0));
            __builtin___memset_chk (resp, 0, length * sizeof(*resp), __builtin_object_size (resp, 0));
        }
...
static double coef_sf2zf(double *a, int N, int n)
{
    ...
    for (int i = 0; i <= N; i++) {
        ...
        for (int k = ((n - N + i) > (0) ? (n - N + i) : (0)); k <= ((i) > (n) ? (n) : (i)); k++) {
            acc += ((fact(i) * fact(N - i)) /
...
static void convert_sf2tf(AVFilterContext *ctx, int channels)
{
    ...
    for (ch = 0; ch < channels; ch++) {
        ...
        __builtin___memcpy_chk (temp0, iir->ab[0], iir->nb_ab[0] * sizeof(*temp0), __builtin_object_size (temp0, 0));
        __builtin___memcpy_chk (temp1, iir->ab[1], iir->nb_ab[1] * sizeof(*temp1), __builtin_object_size (temp1, 0));
...
static void convert_pd2zp(AVFilterContext *ctx, int channels)
{
    ...
    for (ch = 0; ch < channels; ch++) {
        ...
        for (n = 0; n < iir->nb_ab[0]; n++) {
            double r = iir->ab[0][2*n];
            double angle = 3.14159265358979323846264338327950288*iir->ab[0][2*n+1]/180.;
        ...
        for (n = 0; n < iir->nb_ab[1]; n++) {
            double r = iir->ab[1][2*n];
            double angle = 3.14159265358979323846264338327950288*iir->ab[1][2*n+1]/180.;
...
static void check_stability(AVFilterContext *ctx, int channels)
{
    ...
    for (ch = 0; ch < channels; ch++) {
        ...
        for (int n = 0; n < iir->nb_ab[0]; n++) {
            ...
            if (pr >= 1.) {
                av_log(ctx, 24, "pole %d at channel %d is unstable\n", n, ch);
                break;
            }
        }
    }
...
static void draw_line(AVFrame *out, int x0, int y0, int x1, int y1, uint32_t color)
{
    ...
    int dy = ((y1-y0) >= 0 ? (y1-y0) : (-(y1-y0))), sy = y0 < y1 ? 1 : -1;
...
{
    AudioIIRContext *s = ctx->priv;
    double *mag, *phase, *temp, *delay, min = 1.7976931348623157e+308, max = -1.7976931348623157e+308;
    double min_delay = 1.7976931348623157e+308, max_delay = -1.7976931348623157e+308, min_phase, max_phase;
    int prev_ymag = -1, prev_yphase = -1, prev_ydelay = -1;
    char text[32];
    int ch, i;

    __builtin___memset_chk (out->data[0], 0, s->h * out->linesize[0], __builtin_object_size (out->data[0], 0));
    ...
    for (i = 0; i < s->w - 1; i++) {
        ...
        temp[i + 1] = ceil(fabs(d) / (2. * 3.14159265358979323846264338327950288)) * 2. * 3.14159265358979323846264338327950288 * ((d > 3.14159265358979323846264338327950288) - (d < -3.14159265358979323846264338327950288));
    }
    ...
    for (i = 0; i < s->w; i++) {
        ...
        ymag = s->h - 1 - av_clip_c(ymag, 0, s->h - 1);
        yphase = s->h - 1 - av_clip_c(yphase, 0, s->h - 1);
        ydelay = s->h - 1 - av_clip_c(ydelay, 0, s->h - 1);
        ...
        draw_line(out, i, ymag, ((i - 1) > (0) ? (i - 1) : (0)), prev_ymag, 0xFFFF00FF);
        draw_line(out, i, yphase, ((i - 1) > (0) ? (i - 1) : (0)), prev_yphase, 0xFF00FF00);
        draw_line(out, i, ydelay, ((i - 1) > (0) ? (i - 1) : (0)), prev_ydelay, 0xFF00FFFF);
    ...
    if (s->w > 400 && s->h > 100) {
        ...
        __builtin___snprintf_chk (text, sizeof(text), 0, __builtin_object_size (text, 2 > 1 ? 1 : 0), "%.2f", max);
        drawtext(out, 15 * 8 + 2, 2, text, 0xDDDDDDDD);

        drawtext(out, 2, 12, "Min Magnitude:", 0xDDDDDDDD);
        __builtin___snprintf_chk (text, sizeof(text), 0, __builtin_object_size (text, 2 > 1 ? 1 : 0), "%.2f", min);
        drawtext(out, 15 * 8 + 2, 12, text, 0xDDDDDDDD);

        drawtext(out, 2, 22, "Max Phase:", 0xDDDDDDDD);
        __builtin___snprintf_chk (text, sizeof(text), 0, __builtin_object_size (text, 2 > 1 ? 1 : 0), "%.2f", max_phase);
        drawtext(out, 15 * 8 + 2, 22, text, 0xDDDDDDDD);

        drawtext(out, 2, 32, "Min Phase:", 0xDDDDDDDD);
        __builtin___snprintf_chk (text, sizeof(text), 0, __builtin_object_size (text, 2 > 1 ? 1 : 0), "%.2f", min_phase);
        drawtext(out, 15 * 8 + 2, 32, text, 0xDDDDDDDD);

        drawtext(out, 2, 42, "Max Delay:", 0xDDDDDDDD);
        __builtin___snprintf_chk (text, sizeof(text), 0, __builtin_object_size (text, 2 > 1 ? 1 : 0), "%.2f", max_delay);
        drawtext(out, 11 * 8 + 2, 42, text, 0xDDDDDDDD);

        drawtext(out, 2, 52, "Min Delay:", 0xDDDDDDDD);
        __builtin___snprintf_chk (text, sizeof(text), 0, __builtin_object_size (text, 2 > 1 ? 1 : 0), "%.2f", min_delay);
...
static int config_output(AVFilterLink *outlink)
{
    ...
    if (s->format == 0)
        av_log(ctx, 24, "transfer function coefficients format is not recommended for too high number of zeros/poles.\n");

    if (s->format > 0 && s->process == 0) {
        av_log(ctx, 24, "Direct processing is not recommended for zp coefficients format.\n");

        ret = convert_zp2tf(ctx, inlink->ch_layout.nb_channels);
        if (ret < 0)
            return ret;
    } else if (s->format == -2 && s->process > 0) {
        av_log(ctx, 16, "Only direct processing is implemented for lattice-ladder function.\n");
        return (-(int)(('P') | (('A') << 8) | (('W') << 16) | ((unsigned)('E') << 24)));
    } else if (s->format <= 0 && s->process == 1) {
        av_log(ctx, 16, "Serial processing is not implemented for transfer function.\n");
        return (-(int)(('P') | (('A') << 8) | (('W') << 16) | ((unsigned)('E') << 24)));
    } else if (s->format <= 0 && s->process == 2) {
        av_log(ctx, 16, "Parallel processing is not implemented for transfer function.\n");
        return (-(int)(('P') | (('A') << 8) | (('W') << 16) | ((unsigned)('E') << 24)));
    } else if (s->format > 0 && s->process == 1) {
        ...
        if (s->precision > 1)
            av_log(ctx, 24, "Parallel processing is not recommended for fixed-point precisions.\n");
        ret = decompose_zp2biquads(ctx, inlink->ch_layout.nb_channels);
        if (ret < 0)
            return ret;
    ...
    for (ch = 0; s->format == -2 && ch < inlink->ch_layout.nb_channels; ch++) {
        ...
        if (iir->nb_ab[0] != iir->nb_ab[1] + 1) {
            av_log(ctx, 16, "Number of ladder coefficients must be one more than number of reflection coefficients.\n");
            return (-(22));
        }
    }
...
static int filter_frame(AVFilterLink *inlink, AVFrame *in)
{
    ...
    td.in = in;
    td.out = out;
    ff_filter_execute(ctx, s->iir_channel, &td, ((void*)0), outlink->ch_layout.nb_channels);

    for (ch = 0; ch < outlink->ch_layout.nb_channels; ch++) {
        if (s->iir[ch].clippings > 0)
            av_log(ctx, 24, "Channel %d clipping %d times. Please reduce gain.\n",
...
static const AVOption aiir_options[] = {
    { "zeros", "set B/numerator/zeros/reflection coefficients", __builtin_offsetof(AudioIIRContext, b_str), AV_OPT_TYPE_STRING, {.str="1+0i 1-0i"}, 0, 0, 8|(1<<16) },
    { "z", "set B/numerator/zeros/reflection coefficients", __builtin_offsetof(AudioIIRContext, b_str), AV_OPT_TYPE_STRING, {.str="1+0i 1-0i"}, 0, 0, 8|(1<<16) },
    { "poles", "set A/denominator/poles/ladder coefficients", __builtin_offsetof(AudioIIRContext, a_str), AV_OPT_TYPE_STRING, {.str="1+0i 1-0i"}, 0, 0, 8|(1<<16) },
    { "p", "set A/denominator/poles/ladder coefficients", __builtin_offsetof(AudioIIRContext, a_str), AV_OPT_TYPE_STRING, {.str="1+0i 1-0i"}, 0, 0, 8|(1<<16) },
    { "gains", "set channels gains", __builtin_offsetof(AudioIIRContext, g_str), AV_OPT_TYPE_STRING, {.str="1|1"}, 0, 0, 8|(1<<16) },
    { "k", "set channels gains", __builtin_offsetof(AudioIIRContext, g_str), AV_OPT_TYPE_STRING, {.str="1|1"}, 0, 0, 8|(1<<16) },
    { "dry", "set dry gain", __builtin_offsetof(AudioIIRContext, dry_gain), AV_OPT_TYPE_DOUBLE, {.dbl=1}, 0, 1, 8|(1<<16) },
    { "wet", "set wet gain", __builtin_offsetof(AudioIIRContext, wet_gain), AV_OPT_TYPE_DOUBLE, {.dbl=1}, 0, 1, 8|(1<<16) },
    { "format", "set coefficients format", __builtin_offsetof(AudioIIRContext, format), AV_OPT_TYPE_INT, {.i64=1}, -2, 4, 8|(1<<16), "format" },
    { "f", "set coefficients format", __builtin_offsetof(AudioIIRContext, format), AV_OPT_TYPE_INT, {.i64=1}, -2, 4, 8|(1<<16), "format" },
    { "ll", "lattice-ladder function", 0, AV_OPT_TYPE_CONST, {.i64=-2}, 0, 0, 8|(1<<16), "format" },
    { "sf", "analog transfer function", 0, AV_OPT_TYPE_CONST, {.i64=-1}, 0, 0, 8|(1<<16), "format" },
    { "tf", "digital transfer function", 0, AV_OPT_TYPE_CONST, {.i64=0}, 0, 0, 8|(1<<16), "format" },
    { "zp", "Z-plane zeros/poles", 0, AV_OPT_TYPE_CONST, {.i64=1}, 0, 0, 8|(1<<16), "format" },
    { "pr", "Z-plane zeros/poles (polar radians)", 0, AV_OPT_TYPE_CONST, {.i64=2}, 0, 0, 8|(1<<16), "format" },
    { "pd", "Z-plane zeros/poles (polar degrees)", 0, AV_OPT_TYPE_CONST, {.i64=3}, 0, 0, 8|(1<<16), "format" },
    { "sp", "S-plane zeros/poles", 0, AV_OPT_TYPE_CONST, {.i64=4}, 0, 0, 8|(1<<16), "format" },
    { "process", "set kind of processing", __builtin_offsetof(AudioIIRContext, process), AV_OPT_TYPE_INT, {.i64=1}, 0, 2, 8|(1<<16), "process" },
    { "r", "set kind of processing", __builtin_offsetof(AudioIIRContext, process), AV_OPT_TYPE_INT, {.i64=1}, 0, 2, 8|(1<<16), "process" },
    { "d", "direct", 0, AV_OPT_TYPE_CONST, {.i64=0}, 0, 0, 8|(1<<16), "process" },
    { "s", "serial", 0, AV_OPT_TYPE_CONST, {.i64=1}, 0, 0, 8|(1<<16), "process" },
    { "p", "parallel", 0, AV_OPT_TYPE_CONST, {.i64=2}, 0, 0, 8|(1<<16), "process" },
    { "precision", "set filtering precision", __builtin_offsetof(AudioIIRContext, precision),AV_OPT_TYPE_INT, {.i64=0}, 0, 3, 8|(1<<16), "precision" },
    { "e", "set precision", __builtin_offsetof(AudioIIRContext, precision),AV_OPT_TYPE_INT, {.i64=0}, 0, 3, 8|(1<<16), "precision" },
    { "dbl", "double-precision floating-point", 0, AV_OPT_TYPE_CONST, {.i64=0}, 0, 0, 8|(1<<16), "precision" },
    { "flt", "single-precision floating-point", 0, AV_OPT_TYPE_CONST, {.i64=1}, 0, 0, 8|(1<<16), "precision" },
    { "i32", "32-bit integers", 0, AV_OPT_TYPE_CONST, {.i64=2}, 0, 0, 8|(1<<16), "precision" },
    { "i16", "16-bit integers", 0, AV_OPT_TYPE_CONST, {.i64=3}, 0, 0, 8|(1<<16), "precision" },
    { "normalize", "normalize coefficients", __builtin_offsetof(AudioIIRContext, normalize),AV_OPT_TYPE_BOOL, {.i64=1}, 0, 1, 8|(1<<16) },
    { "n", "normalize coefficients", __builtin_offsetof(AudioIIRContext, normalize),AV_OPT_TYPE_BOOL, {.i64=1}, 0, 1, 8|(1<<16) },
    { "mix", "set mix", __builtin_offsetof(AudioIIRContext, mix), AV_OPT_TYPE_DOUBLE, {.dbl=1}, 0, 1, 8|(1<<16) },
    { "response", "show IR frequency response", __builtin_offsetof(AudioIIRContext, response), AV_OPT_TYPE_BOOL, {.i64=0}, 0, 1, 16|(1<<16) },
    { "channel", "set IR channel to display frequency response", __builtin_offsetof(AudioIIRContext, ir_channel), AV_OPT_TYPE_INT, {.i64=0}, 0, 1024, 16|(1<<16) },
    { "size", "set video size", __builtin_offsetof(AudioIIRContext, w), AV_OPT_TYPE_IMAGE_SIZE, {.str = "hd720"}, 0, 0, 16|(1<<16) },
    { "rate", "set video rate", __builtin_offsetof(AudioIIRContext, rate), AV_OPT_TYPE_VIDEO_RATE, {.str = "25"}, 0, 2147483647, 16|(1<<16) },
    { ((void*)0) },
};

static const AVClass aiir_class = { .class_name = "aiir", .item_name = av_default_item_name, .option = aiir_options, .version = ((58)<<16 | (36)<<8 | (100)), .category = AV_CLASS_CATEGORY_FILTER, };
