...
static __attribute__((cold)) int init(AVFilterContext *ctx)
{
    ...
    if (scale->w_expr && !scale->h_expr)
        do{char * SWAP_tmp= scale->size_str; scale->size_str= scale->w_expr; scale->w_expr= SWAP_tmp;}while(0);
    ...
    if (scale->size_str) {
        ...
        __builtin___snprintf_chk (buf, sizeof(buf)-1, 0, __builtin_object_size (buf, 2 > 1 ? 1 : 0), "%d", scale->w);
        av_opt_set(scale, "w", buf, 0);
        __builtin___snprintf_chk (buf, sizeof(buf)-1, 0, __builtin_object_size (buf, 2 > 1 ? 1 : 0), "%d", scale->h);
    ...
    ret = scale_parse_expr(ctx, ((void*)0), &scale->w_pexpr, "width", scale->w_expr);
    if (ret < 0)
        return ret;

    ret = scale_parse_expr(ctx, ((void*)0), &scale->h_pexpr, "height", scale->h_expr);
    if (ret < 0)
        return ret;
    ...
    if (inlink0->w == outlink->w &&
        ...
        for (i = 0; i < 3; i++) {
            ...
            *swscs[i] = s;
            ...
            if ((ret = sws_init_context(s, ((void*)0), ((void*)0))) < 0)
                return ret;
            if (!scale->interlaced)
                break;
        }
...
const AVFilter ff_vf_scale = {
    ...
    .inputs = (avfilter_vf_scale_inputs), .nb_inputs = (sizeof((avfilter_vf_scale_inputs)) / sizeof(((avfilter_vf_scale_inputs))[0])),
    .outputs = (avfilter_vf_scale_outputs), .nb_outputs = (sizeof((avfilter_vf_scale_outputs)) / sizeof(((avfilter_vf_scale_outputs))[0])),
...
const AVFilter ff_vf_scale2ref = {
    ...
    .priv_size = sizeof(ScaleContext),
    .priv_class = &scale_class,
    .inputs = (avfilter_vf_scale2ref_inputs), .nb_inputs = (sizeof((avfilter_vf_scale2ref_inputs)) / sizeof(((avfilter_vf_scale2ref_inputs))[0])),
    .outputs = (avfilter_vf_scale2ref_outputs), .nb_outputs = (sizeof((avfilter_vf_scale2ref_outputs)) / sizeof(((avfilter_vf_scale2ref_outputs))[0])),
    .formats.query_func = query_formats, .formats_state = FF_FILTER_FORMATS_QUERY_FUNC,
    .process_command = process_command,
};
